...
static int is_data32(IR1_INST *pir1)
{
    int dflag = 32;
#ifdef TARGET_X86_64
    if (ir1_rex_bd(pir1) & 0x8) {
        dflag = 64;
    } else if (ir1_prefix_opnd_size_bd(pir1)) {
        dflag = 16;
    }
#else
    if (ir1_prefix_opnd_size_bd(pir1)) {
        dflag = 16;
    }
#endif
    return dflag == 32 ? 1 : 0;
...
static void la_update_fp_status(IR2_OPND cw_opnd)
{
    ...
    update_fcsr_rm_bd(itemp, tmp_fcsr);
...
static bool translate_wait_softfpu_bd(IR1_INST *pir1)
{
    gen_softfpu_helper1((ADDR)helper_fwait);
    return true;
}

static bool translate_f2xm1_softfpu_bd(IR1_INST *pir1)
{
    gen_softfpu_helper1((ADDR)helper_f2xm1);
    return true;
}

static bool translate_fabs_softfpu_bd(IR1_INST *pir1)
{
    if (option_softfpu == 2) {
...
static void gen_float64_STN_ST0(IR1_INST *pir1, int dest, int src)
{
    IR2_OPND dest_opnd = ra_alloc_ftemp();
    IR2_OPND src_opnd = ra_alloc_ftemp();

    get_float80_from_stn(dest_opnd, dest);
    get_float80_from_stn(src_opnd, src);
    la_calcute_float64(pir1, dest_opnd, src_opnd, true);
    save_float80_to_stn(dest_opnd, dest);
...
static bool translate_faddp_softfpu_bd(IR1_INST *pir1)
{
    IR1_OPND_BD *src = ir1_get_opnd_bd(pir1, 0);
	int stn = ir1_opnd_base_reg_num_bd(src);
...
static bool translate_fadd_softfpu_bd(IR1_INST *pir1)
{
    lsassert(ir1_opnd_num_bd(pir1) == 2);

    IR1_OPND_BD *src = ir1_get_opnd_bd(pir1, 1);
    if (ir1_opnd_is_mem_bd(src)) {
        int opnd_size = ir1_opnd_size_bd(src);
        IR2_OPND mem_opnd = convert_mem_no_offset_bd(src);
    ...
    int std = ir1_opnd_base_reg_num_bd(dest);
    int stn = ir1_opnd_base_reg_num_bd(src);
    if (option_softfpu == 2 && option_softfpu_fast & 0x2) {
        gen_float64_STN_ST0(pir1, std, stn);
        return true;
    } else {
        if (stn == 0) {
            gen_softfpu_helper2i((ADDR)helper_fadd_STN_ST0, std);
            return true;
        } else {
...
static bool translate_fbld_softfpu_bd(IR1_INST *pir1)
{
...
static bool translate_fbstp_softfpu_bd(IR1_INST *pir1)
{
...
static bool translate_fchs_softfpu_bd(IR1_INST *pir1)
{
    if (option_softfpu == 2) {
        IR2_OPND high_opnd = ra_alloc_itemp();
        IR2_OPND itemp = ra_alloc_itemp();
...
static bool translate_fcmovnb_softfpu_bd(IR1_INST *pir1)
{
	...
	int stn = ir1_opnd_base_reg_num_bd(opnd1);
...
static bool translate_fcmovb_softfpu_bd(IR1_INST *pir1)
{
    IR2_OPND label_exit = ra_alloc_label();
    IR2_OPND flag_opnd = ra_alloc_itemp();
    la_x86mfflag(flag_opnd, CF_USEDEF_BIT);
...
static bool translate_fcmovnu_softfpu_bd(IR1_INST *pir1)
{
    IR2_OPND label_exit = ra_alloc_label();
    IR2_OPND flag_opnd = ra_alloc_itemp();
...
static bool translate_fcmovu_softfpu_bd(IR1_INST *pir1)
{
    IR2_OPND label_exit = ra_alloc_label();
    IR2_OPND flag_opnd = ra_alloc_itemp();
    la_x86mfflag(flag_opnd, PF_USEDEF_BIT);
...
static bool translate_fcmove_softfpu_bd(IR1_INST *pir1)
{
    IR2_OPND label_exit = ra_alloc_label();
    IR2_OPND flag_opnd = ra_alloc_itemp();
...
static bool translate_fcmovne_softfpu_bd(IR1_INST *pir1)
{
    IR2_OPND label_exit = ra_alloc_label();
    IR2_OPND flag_opnd = ra_alloc_itemp();
    la_x86mfflag(flag_opnd, ZF_USEDEF_BIT);
...
static bool translate_fcmovbe_softfpu_bd(IR1_INST *pir1)
{
    IR2_OPND label_exit = ra_alloc_label();
    IR2_OPND flag_opnd = ra_alloc_itemp();
...
static bool translate_fcmovnbe_softfpu_bd(IR1_INST *pir1)
{
    IR2_OPND label_exit = ra_alloc_label();
    IR2_OPND flag_opnd = ra_alloc_itemp();
    la_x86mfflag(flag_opnd, ZF_USEDEF_BIT | CF_USEDEF_BIT);
...
static bool translate_fcom_softfpu_bd(IR1_INST *pir1)
{
    IR1_OPND_BD *opnd0 = ir1_get_opnd_bd(pir1, 1);
    int opnd_size = ir1_opnd_size_bd(opnd0);
    if (ir1_opnd_is_fpr_bd(opnd0)) {
        int stn = ir1_opnd_base_reg_num_bd(opnd0);
        gen_softfpu_helper2i((ADDR)helper_fmov_FT0_STN, stn);
        gen_softfpu_helper1((ADDR)helper_fcom_ST0_FT0);
    } else if (ir1_opnd_is_mem_bd(opnd0) && opnd_size == 32) {
        IR2_OPND mem_opnd = convert_mem_no_offset_bd(opnd0);
        gen_softfpu_helper2m_32s((ADDR)helper_flds_FT0, mem_opnd);
        gen_softfpu_helper1((ADDR)helper_fcom_ST0_FT0);
    } else if (ir1_opnd_is_mem_bd(opnd0) && opnd_size == 64) {
        IR2_OPND mem_opnd = convert_mem_no_offset_bd(opnd0);
        gen_softfpu_helper2m_64((ADDR)helper_fldl_FT0, mem_opnd);
        gen_softfpu_helper1((ADDR)helper_fcom_ST0_FT0);
    } else {
...
static bool translate_fcomi_softfpu_bd(IR1_INST *pir1)
{
    IR1_OPND_BD *opnd0 = ir1_get_opnd_bd(pir1, 1);
    int stn = ir1_opnd_base_reg_num_bd(opnd0);
    gen_softfpu_helper2i((ADDR)helper_fmov_FT0_STN, stn);
...
static bool translate_fcomip_softfpu_bd(IR1_INST *pir1)
{
    IR1_OPND_BD *opnd0 = ir1_get_opnd_bd(pir1, 1);
    int stn = ir1_opnd_base_reg_num_bd(opnd0);
    gen_softfpu_helper2i((ADDR)helper_fmov_FT0_STN, stn);
...
static bool translate_fcomp_softfpu_bd(IR1_INST *pir1)
{
    int opnd_num = ir1_opnd_num_bd(pir1);
    IR1_OPND_BD *opnd0 = ir1_get_opnd_bd(pir1, 1);
    int opnd_size = ir1_opnd_size_bd(opnd0);
    if (ir1_opnd_is_fpr_bd(opnd0)) {
        int stn = 1;
        if (opnd_num) {
            stn = ir1_opnd_base_reg_num_bd(opnd0);
        }
        gen_softfpu_helper2i((ADDR)helper_fmov_FT0_STN, stn);
        gen_softfpu_helper1((ADDR)helper_fcom_ST0_FT0);
    } else if (ir1_opnd_is_mem_bd(opnd0) && opnd_size == 32) {
...
static bool translate_fcompp_softfpu_bd(IR1_INST *pir1)
{
...
static bool translate_fcos_softfpu_bd(IR1_INST *pir1)
{
    gen_softfpu_helper1((ADDR)helper_fcos);
    return true;
}

static bool translate_fdecstp_softfpu_bd(IR1_INST *pir1)
{
    if (option_softfpu == 2) {
        IR2_OPND top_opnd = ra_alloc_itemp();
...
static bool translate_fdiv_softfpu_bd(IR1_INST *pir1)
{
    lsassert(ir1_opnd_num_bd(pir1) == 2);
    ...
    if (option_softfpu == 2 && option_softfpu_fast & 0x4) {
        ...
        if (stn == 0) {
            gen_softfpu_helper2i((ADDR)helper_fdiv_STN_ST0, std);
            return true;
        } else {
            gen_softfpu_helper2i((ADDR)helper_fmov_FT0_STN, stn);
            gen_softfpu_helper1((ADDR)helper_fdiv_ST0_FT0);
...
static bool translate_fdivp_softfpu_bd(IR1_INST *pir1)
{
    IR1_OPND_BD *src = ir1_get_opnd_bd(pir1, 0);
	int stn = ir1_opnd_base_reg_num_bd(src);
...
static bool translate_fdivr_softfpu_bd(IR1_INST *pir1)
{
    lsassert(ir1_opnd_num_bd(pir1) == 2);
    ...
    if (option_softfpu == 2 && option_softfpu_fast & 0x10) {
        ...
        if (stn == 0) {
            gen_softfpu_helper2i((ADDR)helper_fdivr_STN_ST0, std);
            return true;
        } else {
            gen_softfpu_helper2i((ADDR)helper_fmov_FT0_STN, stn);
            gen_softfpu_helper1((ADDR)helper_fdivr_ST0_FT0);
...
static bool translate_fdivrp_softfpu_bd(IR1_INST *pir1)
{
    IR1_OPND_BD *src = ir1_get_opnd_bd(pir1, 0);
	int stn = ir1_opnd_base_reg_num_bd(src);
...
static bool translate_ffree_softfpu_bd(IR1_INST *pir1)
{
    if (option_softfpu == 2) {
        IR2_OPND fpu_tag = ra_alloc_itemp();
        IR2_OPND top_opnd = ra_alloc_itemp();
        IR2_OPND temp1 = ra_alloc_itemp();
        IR2_OPND temp2 = ra_alloc_itemp();
        IR1_OPND_BD *sti_reg = ir1_get_opnd_bd(pir1, 0);
        ...
        switch (sti_reg->Info.Register.Reg) {
            case NDR_ST0: {
                break;
            }
            case NDR_ST1: {
...
static bool translate_ffreep_softfpu_bd(IR1_INST *pir1)
{
    if (option_softfpu == 2) {
        translate_ffree_softfpu_bd(pir1);
        la_fpu_pop();
    } else {
        IR1_OPND_BD *opnd0 = ir1_get_opnd_bd(pir1, 0);
        int stn = ir1_opnd_base_reg_num_bd(opnd0);
...
static bool translate_fiadd_softfpu_bd(IR1_INST *pir1)
{
    IR1_OPND_BD *opnd0 = ir1_get_opnd_bd(pir1, 1);
    int opnd_size = ir1_opnd_size_bd(opnd0);
    IR2_OPND mem_opnd = convert_mem_no_offset_bd(opnd0);
...
static bool translate_ficom_softfpu_bd(IR1_INST *pir1)
{
    IR1_OPND_BD *opnd0 = ir1_get_opnd_bd(pir1, 1);
    int opnd_size = ir1_opnd_size_bd(opnd0);
    IR2_OPND mem_opnd = convert_mem_no_offset_bd(opnd0);
...
static bool translate_ficomp_softfpu_bd(IR1_INST *pir1)
{
    IR1_OPND_BD *opnd0 = ir1_get_opnd_bd(pir1, 1);
    int opnd_size = ir1_opnd_size_bd(opnd0);
    IR2_OPND mem_opnd = convert_mem_no_offset_bd(opnd0);
...
static bool translate_fidiv_softfpu_bd(IR1_INST *pir1)
{
    IR1_OPND_BD *opnd0 = ir1_get_opnd_bd(pir1, 1);
    int opnd_size = ir1_opnd_size_bd(opnd0);
    IR2_OPND mem_opnd = convert_mem_no_offset_bd(opnd0);
...
static bool translate_fidivr_softfpu_bd(IR1_INST *pir1)
{
    IR1_OPND_BD *opnd0 = ir1_get_opnd_bd(pir1, 1);
    int opnd_size = ir1_opnd_size_bd(opnd0);
    IR2_OPND mem_opnd = convert_mem_no_offset_bd(opnd0);
...
static bool translate_fild_softfpu_bd(IR1_INST *pir1)
{
    IR1_OPND_BD *opnd0 = ir1_get_opnd_bd(pir1, 1);
    int opnd_size = ir1_opnd_size_bd(opnd0);
    IR2_OPND mem_opnd = convert_mem_no_offset_bd(opnd0);
...
static bool translate_fimul_softfpu_bd(IR1_INST *pir1)
{
    IR1_OPND_BD *opnd0 = ir1_get_opnd_bd(pir1, 1);
    int opnd_size = ir1_opnd_size_bd(opnd0);
    IR2_OPND mem_opnd = convert_mem_no_offset_bd(opnd0);
...
static bool translate_fincstp_softfpu_bd(IR1_INST *pir1)
{
    if (option_softfpu == 2) {
        IR2_OPND top_opnd = ra_alloc_itemp();
...
static bool translate_fist_softfpu_bd(IR1_INST *pir1)
{
    IR1_OPND_BD *opnd0 = ir1_get_opnd_bd(pir1, 0);
    int opnd_size = ir1_opnd_size_bd(opnd0);
    ...
    if (option_softfpu == 2 && option_softfpu_fast & 0x200000) {
        ...
        if (option_mem_test) {
            ...
            store_ireg_to_ir1_bd(itemp, opnd0, false);
        } else {
            store_ireg_to_ir1_bd(a0_ir2_opnd, opnd0, false);
        }
...
static bool translate_fistp_softfpu_bd(IR1_INST *pir1)
{
...
static bool translate_fisttp_softfpu_bd(IR1_INST *pir1)
{
    IR1_OPND_BD *opnd0 = ir1_get_opnd_bd(pir1, 0);
    int opnd_size = ir1_opnd_size_bd(opnd0);
...
static bool translate_fisub_softfpu_bd(IR1_INST *pir1)
{
    IR1_OPND_BD *opnd0 = ir1_get_opnd_bd(pir1, 1);
    int opnd_size = ir1_opnd_size_bd(opnd0);
    IR2_OPND mem_opnd = convert_mem_no_offset_bd(opnd0);
...
static bool translate_fisubr_softfpu_bd(IR1_INST *pir1)
{
    IR1_OPND_BD *opnd0 = ir1_get_opnd_bd(pir1, 1);
    int opnd_size = ir1_opnd_size_bd(opnd0);
    IR2_OPND mem_opnd = convert_mem_no_offset_bd(opnd0);
...
static bool translate_fld1_softfpu_bd(IR1_INST *pir1)
{
    if (option_softfpu == 2) {
        IR2_OPND st0_opnd = ra_alloc_ftemp();
        IR2_OPND ftemp = ra_alloc_ftemp();
...
static bool translate_fld_softfpu_bd(IR1_INST *pir1)
{
    if (option_softfpu == 2) {
        IR1_OPND_BD *opnd0 = ir1_get_opnd_bd(pir1, 1);
        int opnd_size = ir1_opnd_size_bd(opnd0);
        ...
        if (ir1_opnd_is_fpr_bd(opnd0)) {
            get_float80_from_stn(st0_opnd, ir1_opnd_base_reg_num_bd(opnd0));
            la_fpu_push();
            save_float80_to_stn(st0_opnd, 0);
            return true;
        } else {
        ...
        if (ir1_opnd_is_fpr_bd(opnd0)) {
            ...
            gen_softfpu_helper2i((ADDR)helper_fmov_ST0_STN,
                    (ir1_opnd_base_reg_num_bd(opnd0) + 1) & 7);
            return true;
        }
...
static bool translate_fldcw_softfpu_bd(IR1_INST *pir1)
{
    IR1_OPND_BD *opnd0 = ir1_get_opnd_bd(pir1, 0);
    IR2_OPND mem_opnd = convert_mem_no_offset_bd(opnd0);
...
static bool translate_fldenv_softfpu_bd(IR1_INST *pir1)
{
    IR1_OPND_BD *opnd0 = ir1_get_opnd_bd(pir1, 0);
    IR2_OPND mem_opnd = convert_mem_no_offset_bd(opnd0);
...
static bool translate_fldl2e_softfpu_bd(IR1_INST *pir1)
{
    if (option_softfpu == 2) {
        IR2_OPND st0_opnd = ra_alloc_ftemp();
        IR2_OPND ftemp = ra_alloc_ftemp();
...
static bool translate_fldl2t_softfpu_bd(IR1_INST *pir1)
{
    if (option_softfpu == 2) {
        IR2_OPND st0_opnd = ra_alloc_ftemp();
        IR2_OPND ftemp = ra_alloc_ftemp();
...
static bool translate_fldlg2_softfpu_bd(IR1_INST *pir1)
{
    if (option_softfpu == 2) {
        IR2_OPND st0_opnd = ra_alloc_ftemp();
        IR2_OPND ftemp = ra_alloc_ftemp();
...
static bool translate_fldln2_softfpu_bd(IR1_INST *pir1)
{
    if (option_softfpu == 2) {
        IR2_OPND st0_opnd = ra_alloc_ftemp();
        IR2_OPND ftemp = ra_alloc_ftemp();
...
static bool translate_fldpi_softfpu_bd(IR1_INST *pir1)
{
    if (option_softfpu == 2) {
        IR2_OPND st0_opnd = ra_alloc_ftemp();
        IR2_OPND ftemp = ra_alloc_ftemp();
...
static bool translate_fldz_softfpu_bd(IR1_INST *pir1)
{
    if (option_softfpu == 2) {
        IR2_OPND st0_opnd = ra_alloc_ftemp();
...
static bool translate_fmul_softfpu_bd(IR1_INST *pir1)
{
    lsassert(ir1_opnd_num_bd(pir1) == 2);
    ...
    if (option_softfpu == 2 && option_softfpu_fast & 0x1000) {
        ...
        if (stn == 0) {
            gen_softfpu_helper2i((ADDR)helper_fmul_STN_ST0, std);
            return true;
        } else {
            gen_softfpu_helper2i((ADDR)helper_fmov_FT0_STN, stn);
            gen_softfpu_helper1((ADDR)helper_fmul_ST0_FT0);
...
static bool translate_fmulp_softfpu_bd(IR1_INST *pir1)
{
    IR1_OPND_BD *src = ir1_get_opnd_bd(pir1, 0);
	int stn = ir1_opnd_base_reg_num_bd(src);
...
static bool translate_fnclex_softfpu_bd(IR1_INST *pir1)
{
    if (option_softfpu == 2) {
...
static bool translate_fninit_softfpu_bd(IR1_INST *pir1)
{
    if (option_softfpu == 2) {
...
static bool translate_fnstenv_softfpu_bd(IR1_INST *pir1)
{
    IR1_OPND_BD *opnd0 = ir1_get_opnd_bd(pir1, 0);
    IR2_OPND mem_opnd = convert_mem_no_offset_bd(opnd0);
...
static bool translate_fnop_softfpu_bd(IR1_INST *pir1)
{
    gen_softfpu_helper1((ADDR)helper_fwait);
    return true;
}

static bool translate_fnsave_softfpu_bd(IR1_INST *pir1)
{
    IR1_OPND_BD *opnd0 = ir1_get_opnd_bd(pir1, 0);
    ...
    if (option_softfpu == 2) {
        translate_fnstenv_softfpu_bd(pir1);
        ...
        ra_free_temp(gpr_stn_low_opnd);
        ra_free_temp(gpr_stn_high_opnd);
        translate_fninit_softfpu_bd(pir1);
...
static bool translate_fnstcw_softfpu_bd(IR1_INST *pir1)
{
    if (option_softfpu == 2) {
        translate_fnstcw_bd(pir1);
    } else {
        IR1_OPND_BD *opnd0 = ir1_get_opnd_bd(pir1, 0);
        gen_softfpu_helper_prologue_bd(pir1);
        ...
        if (ir1_opnd_is_mem_bd(opnd0)) {
            int mem_imm;
            IR2_OPND mem_opnd = convert_mem_bd(opnd0, &mem_imm);
...
static bool translate_fnstsw_softfpu_bd(IR1_INST *pir1)
{
    if (option_softfpu == 2) {
        IR1_OPND_BD *opnd0 = ir1_get_opnd_bd(pir1, 0);
        ...
        if (ir1_opnd_is_mem_bd(opnd0)) {
            ...
            store_ireg_to_ir1_bd(sw_opnd, opnd0, false);
        }
...
static bool translate_fpatan_softfpu_bd(IR1_INST *pir1)
{
    gen_softfpu_helper1((ADDR)helper_fpatan);
    return true;
}

static bool translate_fprem1_softfpu_bd(IR1_INST *pir1)
{
    gen_softfpu_helper1((ADDR)helper_fprem1);
    return true;
}

static bool translate_fprem_softfpu_bd(IR1_INST *pir1)
{
    gen_softfpu_helper1((ADDR)helper_fprem);
    return true;
}

static bool translate_fptan_softfpu_bd(IR1_INST *pir1)
{
    gen_softfpu_helper1((ADDR)helper_fptan);
    return true;
}

static bool translate_frndint_softfpu_bd(IR1_INST *pir1)
{
...
static bool translate_frstor_softfpu_bd(IR1_INST *pir1)
{
    IR1_OPND_BD *opnd0 = ir1_get_opnd_bd(pir1, 0);
    ...
    if (option_softfpu == 2) {
        translate_fldenv_softfpu_bd(pir1);
...
static bool translate_fscale_softfpu_bd(IR1_INST *pir1)
{
...
// static bool translate_fsetpm_softfpu_bd(IR1_INST *pir1)
// {
//     return false;
// }

static bool translate_fsin_softfpu_bd(IR1_INST *pir1)
{
    gen_softfpu_helper1((ADDR)helper_fsin);
    return true;
}

static bool translate_fsincos_softfpu_bd(IR1_INST *pir1)
{
    gen_softfpu_helper1((ADDR)helper_fsincos);
    return true;
}

static bool translate_fsqrt_softfpu_bd(IR1_INST *pir1)
{
...
static bool translate_fst_softfpu_bd(IR1_INST *pir1)
{
    if (option_softfpu == 2 && option_softfpu_fast & 0x800000) {
        ...
        if (ir1_opnd_is_fpr_bd(opnd0)) {
            int opnd0_base_num = ir1_opnd_base_reg_num_bd(opnd0);
...
static bool translate_fstp_softfpu_bd(IR1_INST *pir1)
{
    if (option_softfpu == 2 && option_softfpu_fast & 0x800000) {
        translate_fst_softfpu_bd(pir1);
        la_fpu_pop();
...
static bool translate_fsub_softfpu_bd(IR1_INST *pir1)
{
    lsassert(ir1_opnd_num_bd(pir1) == 2);
    ...
    if (option_softfpu == 2 && option_softfpu_fast & 0x20000) {
        ...
        if (stn == 0) {
            gen_softfpu_helper2i((ADDR)helper_fsub_STN_ST0, std);
            return true;
        } else {
            gen_softfpu_helper2i((ADDR)helper_fmov_FT0_STN, stn);
            gen_softfpu_helper1((ADDR)helper_fsub_ST0_FT0);
...
static bool translate_fsubp_softfpu_bd(IR1_INST *pir1)
{
    IR1_OPND_BD *src = ir1_get_opnd_bd(pir1, 0);
	int stn = ir1_opnd_base_reg_num_bd(src);
...
static bool translate_fsubr_softfpu_bd(IR1_INST *pir1)
{
    lsassert(ir1_opnd_num_bd(pir1) == 2);
    ...
    if (option_softfpu == 2 && option_softfpu_fast & 0x80000) {
        ...
        if (stn == 0) {
            gen_softfpu_helper2i((ADDR)helper_fsubr_STN_ST0, std);
            return true;
        } else {
            gen_softfpu_helper2i((ADDR)helper_fmov_FT0_STN, stn);
            gen_softfpu_helper1((ADDR)helper_fsubr_ST0_FT0);
...
static bool translate_fsubrp_softfpu_bd(IR1_INST *pir1)
{
    IR1_OPND_BD *src = ir1_get_opnd_bd(pir1, 0);
	int stn = ir1_opnd_base_reg_num_bd(src);
...
static bool translate_ftst_softfpu_bd(IR1_INST *pir1)
{
...
static bool translate_fucom_softfpu_bd(IR1_INST *pir1)
{
    IR1_OPND_BD *opnd0 = ir1_get_opnd_bd(pir1, 1);
    int stn = ir1_opnd_base_reg_num_bd(opnd0);
    gen_softfpu_helper2i((ADDR)helper_fmov_FT0_STN, stn);
...
static bool translate_fucomi_softfpu_bd(IR1_INST *pir1)
{
    IR1_OPND_BD *opnd0 = ir1_get_opnd_bd(pir1, 1);
    int stn = ir1_opnd_base_reg_num_bd(opnd0);
    gen_softfpu_helper2i((ADDR)helper_fmov_FT0_STN, stn);
...
static bool translate_fucomip_softfpu_bd(IR1_INST *pir1)
{
    IR1_OPND_BD *opnd0 = ir1_get_opnd_bd(pir1, 1);
    int stn = ir1_opnd_base_reg_num_bd(opnd0);
    gen_softfpu_helper2i((ADDR)helper_fmov_FT0_STN, stn);
...
static bool translate_fucomp_softfpu_bd(IR1_INST *pir1)
{
    IR1_OPND_BD *opnd0 = ir1_get_opnd_bd(pir1, 1);
    int stn = ir1_opnd_base_reg_num_bd(opnd0);
    gen_softfpu_helper2i((ADDR)helper_fmov_FT0_STN, stn);
...
static bool translate_fucompp_softfpu_bd(IR1_INST *pir1)
{
    gen_softfpu_helper2i((ADDR)helper_fmov_FT0_STN, 1);
    gen_softfpu_helper1((ADDR)helper_fucom_ST0_FT0);
    gen_softfpu_helper1((ADDR)helper_fpop);
...
static bool translate_fxam_softfpu_bd(IR1_INST *pir1)
{
    if (option_softfpu == 2) {
        IR2_OPND fpr_st0_low_opnd = ra_alloc_ftemp();
        IR2_OPND ftemp = ra_alloc_ftemp();
...
static bool translate_fxch_softfpu_bd(IR1_INST *pir1)
{
    IR1_OPND_BD *src = ir1_get_opnd_bd(pir1, 1);
	int stn = ir1_opnd_base_reg_num_bd(src);
    if (option_softfpu == 2) {
...
static bool translate_fxrstor_softfpu_bd(IR1_INST *pir1)
{
    IR1_OPND_BD *opnd0 = ir1_get_opnd_bd(pir1, 0);
    IR2_OPND mem_opnd = convert_mem_no_offset_bd(opnd0);
...
static bool translate_fxsave_softfpu_bd(IR1_INST *pir1)
{
    IR1_OPND_BD *opnd0 = ir1_get_opnd_bd(pir1, 0);
    IR2_OPND mem_opnd = convert_mem_no_offset_bd(opnd0);
...
static bool translate_fxtract_softfpu_bd(IR1_INST *pir1)
{
    gen_softfpu_helper1((ADDR)helper_fxtract);
    return true;
}

static bool translate_fyl2x_softfpu_bd(IR1_INST *pir1)
{
    gen_softfpu_helper1((ADDR)helper_fyl2x);
    return true;
}

static bool translate_fyl2xp1_softfpu_bd(IR1_INST *pir1)
{
    gen_softfpu_helper1((ADDR)helper_fyl2xp1);
    return true;
}

TRANS_FPU_WRAP_GEN_BD(wait);
TRANS_FPU_WRAP_GEN_BD(f2xm1);
TRANS_FPU_WRAP_GEN_BD(fabs);
TRANS_FPU_WRAP_GEN_BD(fadd);
TRANS_FPU_WRAP_GEN_BD(faddp);
TRANS_FPU_WRAP_GEN_BD(fbld);
TRANS_FPU_WRAP_GEN_BD(fbstp);
TRANS_FPU_WRAP_GEN_BD(fchs);
TRANS_FPU_WRAP_GEN_BD(fcmovnb);
TRANS_FPU_WRAP_GEN_BD(fcmovb);
TRANS_FPU_WRAP_GEN_BD(fcmovnu);
TRANS_FPU_WRAP_GEN_BD(fcmovu);
TRANS_FPU_WRAP_GEN_BD(fcmove);
TRANS_FPU_WRAP_GEN_BD(fcmovne);
TRANS_FPU_WRAP_GEN_BD(fcmovbe);
TRANS_FPU_WRAP_GEN_BD(fcmovnbe);
TRANS_FPU_WRAP_GEN_BD(fcom);
TRANS_FPU_WRAP_GEN_BD(fcomi);
TRANS_FPU_WRAP_GEN_BD(fcomip);
TRANS_FPU_WRAP_GEN_BD(fcomp);
TRANS_FPU_WRAP_GEN_BD(fcompp);
TRANS_FPU_WRAP_GEN_BD(fcos);
TRANS_FPU_WRAP_GEN_BD(fdecstp);
TRANS_FPU_WRAP_GEN_BD(fdiv);
TRANS_FPU_WRAP_GEN_BD(fdivp);
TRANS_FPU_WRAP_GEN_BD(fdivr);
TRANS_FPU_WRAP_GEN_BD(fdivrp);
TRANS_FPU_WRAP_GEN_BD(ffree);
TRANS_FPU_WRAP_GEN_BD(ffreep);
TRANS_FPU_WRAP_GEN_BD(fiadd);
TRANS_FPU_WRAP_GEN_BD(ficom);
TRANS_FPU_WRAP_GEN_BD(ficomp);
TRANS_FPU_WRAP_GEN_BD(fidiv);
TRANS_FPU_WRAP_GEN_BD(fidivr);
TRANS_FPU_WRAP_GEN_BD(fild);
TRANS_FPU_WRAP_GEN_BD(fimul);
TRANS_FPU_WRAP_GEN_BD(fincstp);
TRANS_FPU_WRAP_GEN_NO_PROLOGUE_BD(fist);
TRANS_FPU_WRAP_GEN_NO_PROLOGUE_BD(fistp);
TRANS_FPU_WRAP_GEN_NO_PROLOGUE_BD(fisttp);
TRANS_FPU_WRAP_GEN_BD(fisub);
TRANS_FPU_WRAP_GEN_BD(fisubr);
TRANS_FPU_WRAP_GEN_BD(fld1);
TRANS_FPU_WRAP_GEN_BD(fld);
TRANS_FPU_WRAP_GEN_BD(fldcw);
TRANS_FPU_WRAP_GEN_BD(fldenv);
TRANS_FPU_WRAP_GEN_BD(fldl2e);
TRANS_FPU_WRAP_GEN_BD(fldl2t);
TRANS_FPU_WRAP_GEN_BD(fldlg2);
TRANS_FPU_WRAP_GEN_BD(fldln2);
TRANS_FPU_WRAP_GEN_BD(fldpi);
TRANS_FPU_WRAP_GEN_BD(fldz);
TRANS_FPU_WRAP_GEN_BD(fmul);
TRANS_FPU_WRAP_GEN_BD(fmulp);
TRANS_FPU_WRAP_GEN_BD(fnclex);
TRANS_FPU_WRAP_GEN_BD(fninit);
TRANS_FPU_WRAP_GEN_BD(fnop);
TRANS_FPU_WRAP_GEN_BD(fnsave);
TRANS_FPU_WRAP_GEN_NO_PROLOGUE_BD(fnstcw);
TRANS_FPU_WRAP_GEN_BD(fnstenv);
TRANS_FPU_WRAP_GEN_NO_PROLOGUE_BD(fnstsw);
TRANS_FPU_WRAP_GEN_BD(fpatan);
TRANS_FPU_WRAP_GEN_BD(fprem1);
TRANS_FPU_WRAP_GEN_BD(fprem);
TRANS_FPU_WRAP_GEN_BD(fptan);
TRANS_FPU_WRAP_GEN_BD(frndint);
TRANS_FPU_WRAP_GEN_BD(frstor);
TRANS_FPU_WRAP_GEN_BD(fscale);
// TRANS_FPU_WRAP_GEN_BD(fsetpm);
TRANS_FPU_WRAP_GEN_BD(fsin);
TRANS_FPU_WRAP_GEN_BD(fsincos);
TRANS_FPU_WRAP_GEN_BD(fsqrt);
TRANS_FPU_WRAP_GEN_NO_PROLOGUE_BD(fst);
TRANS_FPU_WRAP_GEN_NO_PROLOGUE_BD(fstp);
TRANS_FPU_WRAP_GEN_BD(fsub);
TRANS_FPU_WRAP_GEN_BD(fsubp);
TRANS_FPU_WRAP_GEN_BD(fsubr);
TRANS_FPU_WRAP_GEN_BD(fsubrp);
TRANS_FPU_WRAP_GEN_BD(ftst);
TRANS_FPU_WRAP_GEN_BD(fucom);
TRANS_FPU_WRAP_GEN_BD(fucomi);
TRANS_FPU_WRAP_GEN_BD(fucomip);
TRANS_FPU_WRAP_GEN_BD(fucomp);
TRANS_FPU_WRAP_GEN_BD(fucompp);
TRANS_FPU_WRAP_GEN_BD(fxam);
TRANS_FPU_WRAP_GEN_BD(fxch);
TRANS_FPU_WRAP_GEN_BD(fxrstor);
TRANS_FPU_WRAP_GEN_BD(fxsave);
TRANS_FPU_WRAP_GEN_BD(fxtract);
TRANS_FPU_WRAP_GEN_BD(fyl2x);
TRANS_FPU_WRAP_GEN_BD(fyl2xp1);
