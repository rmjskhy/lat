...
#include "hbr-bd.h"
bool translate_pop_bd(IR1_INST *pir1)
{
    IR2_OPND esp_opnd = ra_alloc_gpr(esp_index);
    IR1_OPND_BD mem_ir1_opnd;
    int has_esp = ir1_opnd_is_gpr_used_bd(ir1_get_opnd_bd(pir1, 0), esp_index);
    int pop_size = ir1_opnd_size_bd(ir1_get_opnd_bd(pir1, 0));
    int esp_increment = pop_size >> 3;

    if (pop_size == 16 && ir1_opnd_is_seg_bd(ir1_get_opnd_bd(pir1, 0))) {
#ifdef TARGET_X86_64
        if (CODEIS64) {
            esp_increment = 64 >> 3;
        } else {
    ...
    if (CODEIS64) {
        if (pop_size == 64) {/* 64 bits */
            ir1_opnd_build_mem_bd(&mem_ir1_opnd, 64, NDR_RSP, 64, 0);
            if (ir1_opnd_is_gpr_bd(ir1_get_opnd_bd(pir1, 0))) {
                /* when dest is gpr, load into gpr directly */
                IR2_OPND dest_opnd
                        = load_ireg_from_ir1_bd(ir1_get_opnd_bd(pir1, 0), UNKNOWN_EXTENSION, false);
                load_ireg_from_ir1_2_bd(dest_opnd, &mem_ir1_opnd, UNKNOWN_EXTENSION, false);
                if (!has_esp) {
                ...
                /* load value */
                IR2_OPND value_opnd =
                        load_ireg_from_ir1_bd(&mem_ir1_opnd, SIGN_EXTENSION, false);
                IR1_OPND_BD *dest_ir1_opnd = ir1_get_opnd_bd(pir1, 0);
                if (has_esp) {
                    dest_ir1_opnd->Info.Memory.Disp += esp_increment;
                    // dest_ir1_opnd->mem.disp += esp_increment;
                }
                store_ireg_to_ir1_bd(value_opnd, dest_ir1_opnd, false);
                la_addi_addrx(esp_opnd, esp_opnd,
                                        esp_increment);
            }
        } else if (pop_size == 16) {
            ir1_opnd_build_mem_bd(&mem_ir1_opnd, 16, NDR_ESP, 32, 0);
            /* load value */
            IR2_OPND value_opnd =
                load_ireg_from_ir1_bd(&mem_ir1_opnd, ZERO_EXTENSION, false);
            IR1_OPND_BD *dest_ir1_opnd = ir1_get_opnd_bd(pir1, 0);
            if (has_esp) {
                dest_ir1_opnd->Info.Memory.Disp += esp_increment;
                // dest_ir1_opnd->mem.disp += esp_increment;
            ...
            ir1_opnd_build_mem_bd(&mem_ir1_opnd, 32, NDR_ESP, 32, 0);
            if (ir1_opnd_is_gpr_bd(ir1_get_opnd_bd(pir1, 0))) {
                IR2_OPND dest_opnd =
                    load_ireg_from_ir1_bd(ir1_get_opnd_bd(pir1, 0), UNKNOWN_EXTENSION, false);
                ...
                load_ireg_from_ir1_2_bd(dest_opnd, &mem_ir1_opnd, dest_em, false);
                if (!has_esp) {
                    la_addi_addrx(esp_opnd, esp_opnd,
                                                                esp_increment);
                }
...
bool translate_push_bd(IR1_INST *pir1)
{
    IR2_OPND esp_opnd = ra_alloc_gpr(esp_index);
    int push_size = ir1_opnd_size_bd(ir1_get_opnd_bd(pir1, 0));
    int esp_decrement = push_size >> 3;

    if (push_size == 16 && ir1_opnd_is_seg_bd(ir1_get_opnd_bd(pir1, 0))) {
#ifdef TARGET_X86_64
        if (CODEIS64) {
            esp_decrement = 64 >> 3;
        } else {
    ...
    ir1_opnd_build_mem_bd(&mem_ir1_opnd, esp_decrement << 3,
        NDR_ESP, 32, -esp_decrement);
#else
    if (CODEIS64) {
        ir1_opnd_build_mem_bd(&mem_ir1_opnd, esp_decrement << 3,
            NDR_RSP, 64, -esp_decrement);
    } else {
        ir1_opnd_build_mem_bd(&mem_ir1_opnd, esp_decrement << 3,
            NDR_ESP, 32, -esp_decrement);
    }
    ...
    IR2_OPND value_opnd =
        load_ireg_from_ir1_bd(ir1_get_opnd_bd(pir1, 0), UNKNOWN_EXTENSION, false);
    store_ireg_to_ir1_bd(value_opnd, &mem_ir1_opnd, false);
...
static void translate_mov_imm_to_gpr(IR1_OPND_BD *opnd0, IR1_OPND_BD *opnd1)
{
    ulongx imm = ir1_opnd_uimm_bd(opnd1);

    if (imm == 0) {
        store_ireg_to_ir1_bd(zero_ir2_opnd, opnd0, false);
    } else {
        int opnd0_gpr_num = ir1_opnd_base_reg_num_bd(opnd0);
        IR2_OPND dest  = ra_alloc_gpr(opnd0_gpr_num);
        IR2_OPND temp = ra_alloc_itemp();
        uint64_t useful_imm = imm;

        switch (ir1_opnd_size_bd(opnd0)) {
        case 8:
            if (ir1_opnd_is_8l_bd(opnd0)) {
                useful_imm = imm & 0xff;
...
static void translate_mov_from_gpr(IR1_OPND_BD *opnd0, IR1_OPND_BD *opnd1)
{
    int opnd1_gpr_num = ir1_opnd_base_reg_num_bd(opnd1);
    ...
    if (ir1_opnd_is_gpr_bd(opnd0)) {
        ...
        int opnd0_gpr_num = ir1_opnd_base_reg_num_bd(opnd0);
        IR2_OPND dest  = ra_alloc_gpr(opnd0_gpr_num);
        switch (ir1_opnd_size_bd(opnd1)) {
        case 8:
            if (ir1_opnd_is_8l_bd(opnd1)) {
                if (ir1_opnd_is_8l_bd(opnd0)) {
        ...
        if (ir1_opnd_is_8h_bd(opnd1)) {
            IR2_OPND temp = ra_alloc_itemp();
            la_bstrpick_d(temp, src, 15, 8);
            store_ireg_to_ir1_bd(temp, opnd0, false);
            ra_free_temp(temp);
        } else {
            store_ireg_to_ir1_bd(src, opnd0, false);
        }
    }
}

static void translate_mov_from_mem(IR1_OPND_BD *opnd0, IR1_OPND_BD *opnd1)
{
    int opnd0_gpr_num = ir1_opnd_base_reg_num_bd(opnd0);
    IR2_OPND dest  = ra_alloc_gpr(opnd0_gpr_num);
    ...
    switch (ir1_opnd_size_bd(opnd0)) {
    case 8:
        load_ireg_from_ir1_mem_bd(temp, opnd1, UNKNOWN_EXTENSION, false);
        if (ir1_opnd_is_8l_bd(opnd0)) {
            la_bstrins_d(dest, temp, 7, 0);
        } else {
            la_bstrins_d(dest, temp, 15, 8);
    ...
    case 16:
        ...
        break;
    case 32:
        load_ireg_from_ir1_mem_bd(dest, opnd1, ZERO_EXTENSION, false);
        break;
    case 64:
        load_ireg_from_ir1_mem_bd(dest, opnd1, UNKNOWN_EXTENSION, false);
        break;
    default:
        assert(0);
        break;
...
bool translate_mov_bd(IR1_INST *pir1)
{
     ...
     */
    IR2_OPND src_opnd = load_ireg_from_ir1_bd(opnd1, UNKNOWN_EXTENSION, false);
    store_ireg_to_ir1_bd(src_opnd, opnd0, false);
    return true;
}

bool translate_movzx_bd(IR1_INST *pir1)
{
    IR1_OPND_BD *source_ir1 = ir1_get_opnd_bd(pir1, 1);
    IR1_OPND_BD *dest_ir1 = ir1_get_opnd_bd(pir1, 0);
    lsassert(ir1_opnd_is_gpr_bd(dest_ir1));

    int dest_base = ir1_opnd_base_reg_num_bd(dest_ir1);
    ...
    if (ir1_opnd_is_gpr_bd(source_ir1)) {
        /*
         ...
         */
        int source_base = ir1_opnd_base_reg_num_bd(source_ir1);
        IR2_OPND src_opnd = ra_alloc_gpr(source_base);

        if (ir1_opnd_is_8h_bd(source_ir1)) {
            /* MOVZX r16/32/64, r8h */
            if (ir1_opnd_is_16bit_bd(dest_ir1)) {
                /*
                 * MOVZX r16, r8h
                 *
                 * temp <- ZERO(r8h[15:8])
            ...
            if (ir1_opnd_is_16bit_bd(dest_ir1)) {
                ...
                /* MOVZX r32/64, r8l/16 */
                /* 32 bits and 64 bits, all will be clean */
                int opnd_size = ir1_opnd_size_bd(source_ir1);
#ifdef CONFIG_LATX_DEBUG
                if (opnd_size == 8) {
                    lsassert(ir1_opnd_is_8l_bd(source_ir1) || ir1_opnd_is_8h_bd(source_ir1));
                } else if (opnd_size == 16) {
                    lsassert(ir1_opnd_is_16bit_bd(source_ir1));
                } else {
                    lsassert(0);
                }
#endif
     ...
     */
    if (ir1_opnd_size_bd(dest_ir1) >= 32) {
        load_ireg_from_ir1_mem_bd(dest_opnd, source_ir1, ZERO_EXTENSION, false);
    } else {
        /* MOVZX r16, m8 */
        IR2_OPND source_opnd =
            load_ireg_from_ir1_bd(source_ir1, ZERO_EXTENSION, false);

        store_ireg_to_ir1_bd(source_opnd, dest_ir1, false);
    }

    return true;
}

bool translate_movsx_bd(IR1_INST *pir1)
{
    IR2_OPND source_opnd =
        load_ireg_from_ir1_bd(ir1_get_opnd_bd(pir1, 0) + 1, SIGN_EXTENSION, false);

    store_ireg_to_ir1_bd(source_opnd, ir1_get_opnd_bd(pir1, 0), false);
    return true;
}

bool translate_movsxd_bd(IR1_INST *pir1)
{
    lsassert(ir1_opnd_is_gpr_bd(ir1_get_opnd_bd(pir1, 0)));
    int gpr_num = ir1_opnd_base_reg_num_bd(ir1_get_opnd_bd(pir1, 0));
    IR2_OPND gpr_opnd = ra_alloc_gpr(gpr_num);
    load_ireg_from_ir1_2_bd(gpr_opnd, ir1_get_opnd_bd(pir1, 1), SIGN_EXTENSION, false);
#ifdef TARGET_X86_64
    if (CODEIS64 && ir1_opnd_size_bd(ir1_get_opnd_bd(pir1, 0)) == 32)
...
static void load_step_to_reg(IR2_OPND *p_step_opnd, IR1_INST *pir1)
{
    ...
    int bytes = ir1_opnd_size_bd(ir1_get_opnd_bd(pir1, 0)) >> 3;
    int bits = 0;
    switch (ir1_opnd_size_bd(ir1_get_opnd_bd(pir1, 0))) {
    case 8:
        bits = 9;
        break;
    case 16:
...
static void load_step_to_reg_ir1(IR2_OPND *p_step_opnd, IR1_OPND_BD *opnd_ir1)
{
    IR2_OPND df_opnd = ra_alloc_itemp();
    IR2_OPND eflags_opnd = ra_alloc_eflags();
    la_andi(df_opnd, eflags_opnd, 0x400);

    int bytes = ir1_opnd_size_bd(opnd_ir1) >> 3;
    int bits = 0;
    switch (ir1_opnd_size_bd(opnd_ir1)) {
    case 8:
        bits = 9;
        break;
    case 16:
...
bool translate_movs_bd(IR1_INST *pir1)
{
     ...
     */
#ifndef TARGET_X86_64
    clear_h32_bd(&edi_opnd);
    clear_h32_bd(&esi_opnd);
#else
    if (!CODEIS64) {
        clear_h32_bd(&edi_opnd);
        clear_h32_bd(&esi_opnd);
    ...
    if (ir1_prefix_bd(pir1) != 0) {
#ifndef TARGET_X86_64
        clear_h32_bd(&ecx_opnd);
#else
        if (!CODEIS64) {
            clear_h32_bd(&ecx_opnd);
        }
    ...
    IR1_OPND_BD *mem_si = NULL;
    IR1_OPND_BD *mem_di = NULL;
...
//     } else if (ir1_get_opnd_num_bd(pir1) == 2) {
        lsassert(ir1_get_opnd_num_bd(pir1) == 2);
        mem_di = ir1_get_opnd_bd(pir1, 0);
        mem_si = ir1_get_opnd_bd(pir1, 0) + 1;
    // } else {
    //     lsassert(0);
    // }
    ...
    IR2_OPND esi_mem_value =
        load_ireg_from_ir1_bd(mem_si, SIGN_EXTENSION, false);
    store_ireg_to_ir1_bd(esi_mem_value, mem_di, false);
    ...
    if (ir1_prefix_bd(pir1) != 0) {
        lsassert(ir1_prefix_bd(pir1) == ND_PREFIX_G1_REPE_REPZ);
#ifndef TARGET_X86_64
        ...
        store_ireg_to_ir1_bd(ecx_opnd, &ecx_ir1_opnd_bd, false);
    }
...
bool translate_stos_bd(IR1_INST *pir1)
{
    IR2_OPND edi_opnd = ra_alloc_gpr(edi_index);
    ...
    /* 2. preparations outside the loop */
    IR1_OPND_BD *opnd_eax = NULL;
    IR1_OPND_BD *opnd_di = NULL;
//     if (ir1_get_opnd_num_bd(pir1) > 0) {
//         opnd_di = ir1_get_opnd_bd(pir1, 0);
//     }
//     switch (ir1_get_opnd_num_bd(pir1)) {
//     case 0:
//         switch (pir1->info->id) {
//         case dt_X86_INS_STOSB:
//             opnd_eax = &eax_ir1_opnd_bd;
...
//         break;
//     case 2:
//         opnd_eax = ir1_get_opnd_bd(pir1, 0) + 1;
//         break;
//     default:
//         lsassert(0);
//     }
    lsassert(ir1_get_opnd_num_bd(pir1) == 2);
    opnd_di = ir1_get_opnd_bd(pir1, 0);
    opnd_eax = ir1_get_opnd_bd(pir1, 1);
    IR2_OPND eax_value_opnd =
        load_ireg_from_ir1_bd(opnd_eax, SIGN_EXTENSION, false);
    IR2_OPND step_opnd = ra_alloc_itemp();
    ...
    store_ireg_to_ir1_bd(eax_value_opnd, opnd_di, false);
...
bool translate_lods_bd(IR1_INST *pir1)
{
    IR2_OPND esi_opnd = ra_alloc_gpr(esi_index);
    /*
    ...
    if (ir1_opnd_size_bd(ir1_get_opnd_bd(pir1, 0)) < 32)
        em = ZERO_EXTENSION;
    IR2_OPND esi_mem_value = load_ireg_from_ir1_bd(ir1_get_opnd_bd(pir1, 0) + 1, em, false);
    ...
    store_ireg_to_ir1_bd(esi_mem_value, ir1_get_opnd_bd(pir1, 0), false);
...
bool translate_cmps_bd(IR1_INST *pir1)
{
    /* 1. exit when initial count is zero */
    IR2_OPND label_exit = ra_alloc_label();
    IR2_OPND ecx_opnd = ra_alloc_gpr(ecx_index);
    ...
    IR2_OPND esi_mem_value =
        load_ireg_from_ir1_bd(ir1_get_opnd_bd(pir1, 0), SIGN_EXTENSION, false);
    ...
    if (ir1_prefix_bd(pir1) != 0) {
        ...
        if (ir1_prefix_bd(pir1) == ND_PREFIX_G1_REPE_REPZ) {
    ...
    /* 5. calculate eflags */
    generate_eflag_calculation_bd(cmp_result, esi_mem_value, edi_mem_value, pir1,
                               true);
    la_label(label_exit);
...
bool translate_scas_bd(IR1_INST *pir1)
{
    /* 1. exit when initial count is zero */
    IR2_OPND label_exit = ra_alloc_label();
    IR2_OPND ecx_opnd = ra_alloc_gpr(ecx_index);
    ...
    //     break;
    // case 2:
    //     opnd_eax = ir1_get_opnd_bd(pir1, 0);
    //     break;
    // default:
    //     lsassert(0);
    // }
    ...
    // }
    IR2_OPND edi_mem_value =
        load_ireg_from_ir1_bd(ir1_get_opnd_bd(pir1, 1), SIGN_EXTENSION, false);
    /* 3.2 adjust edi */
    IR2_OPND edi_opnd = ra_alloc_gpr(edi_index);
    la_sub(edi_opnd, edi_opnd, step_opnd);
#ifndef TARGET_X86_64
...
#else
        if (CODEIS64) {
            store_ireg_to_ir1_bd(ecx_opnd, &rcx_ir1_opnd_bd, false);
        } else {
            store_ireg_to_ir1_bd(ecx_opnd, &ecx_ir1_opnd_bd, false);
        }
#endif
    ...
    /* 5. calculate eflags */
    generate_eflag_calculation_bd(cmp_result, eax_value_opnd, edi_mem_value, pir1,
                               true);
    la_label(label_exit);
 ...
 */
bool translate_cmovcc_bd(IR1_INST *pir1)
{
    IR1_OPND_BD *opnd0 = ir1_get_opnd_bd(pir1, 0);
    IR1_OPND_BD *opnd1 = ir1_get_opnd_bd(pir1, 1);
    lsassertm(ir1_opnd_is_gpr_bd(opnd0), "CMOVcc need used gpr as dest!");

    IR2_OPND neg_condition = ra_alloc_itemp();

    get_eflag_condition_bd(&neg_condition, pir1);

    /* src maybe a mem */
    IR2_OPND src_opnd = load_ireg_from_ir1_bd(opnd1, UNKNOWN_EXTENSION, false);
    IR2_OPND dest_opnd = ra_alloc_gpr(ir1_opnd_base_reg_num_bd(opnd0));
    ...
    if (ir1_opnd_size_bd(opnd0) == 64) {
        ...
        store_ireg_to_ir1_bd(cond1, opnd0, false);
    }
    ra_free_temp(cond1);
    ra_free_temp(cond2);
...
bool translate_lea_bd(IR1_INST *pir1)
{
    IR1_OPND_BD *op0 = ir1_get_opnd_bd(pir1, 0);
    IR1_OPND_BD *op1 = ir1_get_opnd_bd(pir1, 1);
    int op0_size = ir1_opnd_size_bd(op0);
    int op0_gpr_num = ir1_opnd_base_reg_num_bd(op0);
    IR2_OPND dest_op = ra_alloc_gpr(op0_gpr_num);

    convert_mem_to_specific_gpr_bd(op1, dest_op, op0_size);

    return true;
}

#ifndef CONFIG_LATX_LLSC
static bool translate_xchg_spinlock_bd(IR1_INST *pir1)
{
    IR1_OPND_BD *opnd0 = ir1_get_opnd_bd(pir1, 0);
    IR1_OPND_BD *opnd1 = ir1_get_opnd_bd(pir1, 1);
    IR2_OPND mem_opnd;
    ...
    if (ir1_opnd_is_mem_bd(opnd0)) {
        ...
        mem_opnd = convert_mem_bd(opnd1, &imm);
    }
    ...
    if (ir1_opnd_is_mem_bd(opnd0)) {
        IR2_OPND src_opnd_0 =
            load_ireg_from_ir2_mem(mem_opnd, imm, ir1_opnd_size_bd(opnd0),
                                   UNKNOWN_EXTENSION, false);
        IR2_OPND src_opnd_1 =
            load_ireg_from_ir1_bd(opnd1, UNKNOWN_EXTENSION, false);
        store_ireg_to_ir2_mem(src_opnd_1, mem_opnd, imm, ir1_opnd_size_bd(opnd0), false);
        store_ireg_to_ir1_bd(src_opnd_0, opnd1, false);
    } else if (ir1_opnd_is_mem_bd(opnd1)) {
        IR2_OPND src_opnd_0 =
            load_ireg_from_ir1_bd(opnd0, UNKNOWN_EXTENSION, false);
        IR2_OPND src_opnd_1 =
            load_ireg_from_ir2_mem(mem_opnd, imm, ir1_opnd_size_bd(opnd1),
                                   UNKNOWN_EXTENSION, false);
        store_ireg_to_ir1_bd(src_opnd_1, opnd0, false);
        store_ireg_to_ir2_mem(src_opnd_0, mem_opnd, imm, ir1_opnd_size_bd(opnd1), false);
...
bool translate_xchg_bd(IR1_INST *pir1)
{
    IR1_OPND_BD *opnd0 = ir1_get_opnd_bd(pir1, 0);
    IR1_OPND_BD *opnd1 = ir1_get_opnd_bd(pir1, 1);
    ...
    if ((ir1_opnd_is_gpr_bd(opnd0) && ir1_opnd_is_gpr_bd(opnd1))) {
        /* get src0 */
        src0 = load_ireg_from_ir1_bd(opnd0, UNKNOWN_EXTENSION, false);

        /* get src1 */
        src1 = ra_alloc_itemp();
        load_ireg_from_ir1_2_bd(src1, opnd1, UNKNOWN_EXTENSION, false);

        if (ir1_opnd_is_same_reg_bd(opnd0, opnd1)) {
#ifdef TARGET_X86_64
            if (!GHBR_ON_BD(pir1) && CODEIS64 && opnd0_size == 32) {
        ...
        store_ireg_to_ir1_bd(src0, opnd1, false);
        store_ireg_to_ir1_bd(src1, opnd0, false);
        return true;
    }
    ...
    if (!close_latx_parallel && !(cpu->tcg_cflags & CF_PARALLEL)) {
        /* get src0 and src1*/
        src0 = load_ireg_from_ir1_bd(opnd0, UNKNOWN_EXTENSION, false);
        src1 = load_ireg_from_ir1_bd(opnd1, UNKNOWN_EXTENSION, false);
     ...
     */
#ifndef CONFIG_LATX_LLSC
    return translate_xchg_spinlock_bd(pir1);
#else
    ...
    if (ir1_opnd_is_mem_bd(opnd0)) {
        ...
        IR2_OPND imm_cache_mem_opnd = convert_mem_to_itemp_bd(opnd0);
        la_or(mem_opnd, imm_cache_mem_opnd, zero_ir2_opnd);
#else
        mem_opnd = convert_mem_to_itemp_bd(opnd0);
#endif
        src1 = load_ireg_from_ir1_bd(opnd1, UNKNOWN_EXTENSION, false);
        reg_opnd = opnd1;
    } else {
        ...
        la_or(mem_opnd, imm_cache_mem_opnd, zero_ir2_opnd);
#else
        mem_opnd = convert_mem_to_itemp_bd(opnd1);
#endif
        src1 = load_ireg_from_ir1_bd(opnd0, UNKNOWN_EXTENSION, false);
        reg_opnd = opnd0;
    }
     ...
     */
    la_label(label_exit);
    store_ireg_to_ir1_bd(src0, reg_opnd, false);
    return true;
#endif
}

bool translate_cmpxchg_bd(IR1_INST *pir1)
{
    IR1_OPND_BD *opnd0 = ir1_get_opnd_bd(pir1, 0);
    IR1_OPND_BD *opnd1 = ir1_get_opnd_bd(pir1, 1);
    CPUArchState* env = (CPUArchState*)(lsenv->cpu_state);
    ...
    if (is_lock) {
        return translate_lock_cmpxchg_bd(pir1);
    }
#endif
    ...
    /* get eax */
    if (ir1_opnd_size_bd(opnd0) == 64) {
        reg_ir1 = &rax_ir1_opnd_bd;
    } else if (ir1_opnd_size_bd(opnd0) == 32) {
        reg_ir1 = &eax_ir1_opnd_bd;
    } else if (ir1_opnd_size_bd(opnd0) == 16) {
        reg_ir1 = &ax_ir1_opnd_bd;
    } else if (ir1_opnd_size_bd(opnd0) == 8) {
        reg_ir1 = &al_ir1_opnd_bd;
    }

    /* fast path: cmpxchg rax/eax/ax,r64/r32/r16 */
    if (ir1_opnd_is_same_reg_bd(opnd0, reg_ir1)) {
        eax_opnd = ra_alloc_gpr(eax_index);
        generate_eflag_calculation_bd(eax_opnd, eax_opnd, eax_opnd, pir1, true);
        src1 = load_ireg_from_ir1_bd(opnd1, UNKNOWN_EXTENSION, false);
        store_ireg_to_ir1_bd(src1, opnd0, false);
        return true;
    }

    eax_opnd = load_ireg_from_ir1_bd(reg_ir1, SIGN_EXTENSION, false);

    /* get src0 */
    if (ir1_opnd_is_gpr_bd(opnd0)) {
        src0 = convert_gpr_opnd_bd(opnd0, SIGN_EXTENSION);
    } else {
        src0 = ra_alloc_itemp();
        mem_opnd = convert_mem_bd(opnd0, &imm);
        if (is_lock) {
    ...
    generate_eflag_calculation_bd(src0, eax_opnd, src0, pir1, true);
    ...
    la_dbar(0);
    store_ireg_to_ir1_bd(src0, reg_ir1, false);
    la_label(label_exit);

    /* unlock */
    if (!ir1_opnd_is_gpr_bd(opnd0) && is_lock) {
        tr_lat_spin_unlock(lat_lock_addr);
    }
 ...
 */
bool translate_cmpxchg8b_bd(IR1_INST *pir1)
{
    IR1_OPND_BD *reg_eax = &eax_ir1_opnd_bd, *reg_edx = &edx_ir1_opnd_bd;
    lsassert(ir1_opnd_size_bd(ir1_get_opnd_bd(pir1, 0)) == 64);
    ...
    if (ir1_opnd_simm_bd(opnd0) & 0x3) {
        mem_opnd = convert_mem_no_offset_bd(opnd0);
    } else {
        mem_opnd = convert_mem_bd(opnd0, &imm);
    }
    ir2_set_opnd_type(&mem_opnd, IR2_OPND_GPR);
    ...
    */
    ...
    /* equal */
    generate_eflag_calculation_bd(zero_ir2_opnd, zero_ir2_opnd, zero_ir2_opnd,
                                pir1, true);
    la_b(label_exit);
    ...
    la_label(label_unequal);
    la_dbar(0);
    generate_eflag_calculation_bd(mem_opnd, mem_opnd, zero_ir2_opnd,
                                pir1, true);
    store_ireg_to_ir1_bd(src_opnd_0, reg_eax, false);
    la_srli_d(src_opnd_0, src_opnd_0, 32);
    store_ireg_to_ir1_bd(src_opnd_0, reg_edx, false);
...
static bool translate_cmpxchg16b_scq_bd(IR1_INST *pir1)
{
    IR1_OPND_BD *opnd0 = ir1_get_opnd_bd(pir1, 0);
    IR2_OPND mem_opnd = convert_mem_no_offset_bd(opnd0);
 ...
 */
bool translate_cmpxchg16b_bd(IR1_INST *pir1)
{
    if (have_scq_bd()) {
        return translate_cmpxchg16b_scq_bd(pir1);
    }
    ...
    generate_eflag_calculation_bd(zf_opnd, zf_opnd, zf_opnd,
                                pir1, true);
...
bool translate_movq_bd(IR1_INST *pir1)
{
    ...
    if (ir1_opnd_is_xmm_bd(dest) && ir1_opnd_is_mem_bd(src)) {
        ...
        if (option_enable_lasx) {
            ...
            if (ir1_opnd_base_reg_num_bd(dest) == ir1_opnd_base_reg_num_bd(src)) {
                la_vinsgr2vr_d(ra_alloc_xmm(ir1_opnd_base_reg_num_bd(dest)),
                               zero_ir2_opnd, 1);
            } else {
                la_vandi_b(ra_alloc_xmm(ir1_opnd_base_reg_num_bd(dest)),
                           ra_alloc_xmm(ir1_opnd_base_reg_num_bd(dest)), 0);
        ...
        if (ir1_opnd_is_xmm_bd(dest) && ir1_opnd_is_gpr_bd(src)) {
            lsassert(ir1_opnd_size_bd(src) == 64);
            /* gpr -> xmm */

            la_vandi_b(ra_alloc_xmm(ir1_opnd_base_reg_num_bd(dest)),
                    ra_alloc_xmm(ir1_opnd_base_reg_num_bd(dest)), 0);
            la_vinsgr2vr_d(ra_alloc_xmm(ir1_opnd_base_reg_num_bd(dest)),
                        ra_alloc_gpr(ir1_opnd_base_reg_num_bd(src)), 0);

            return true;
        } else if (ir1_opnd_is_xmm_bd(src) && ir1_opnd_is_gpr_bd(dest)) {
            lsassert(ir1_opnd_size_bd(dest) == 64);
            la_vpickve2gr_du(ra_alloc_gpr(ir1_opnd_base_reg_num_bd(dest)),
                            ra_alloc_xmm(ir1_opnd_base_reg_num_bd(src)), 0);
            return true;
        } else if (ir1_opnd_is_mmx_bd(src) && ir1_opnd_is_gpr_bd(dest)) {
            lsassert(ir1_opnd_size_bd(dest) == 64);
            /* transfer to mmx mode */
            transfer_to_mmx_mode_bd();
            IR2_OPND ir2_src =
                load_ireg_from_ir1_bd(src, UNKNOWN_EXTENSION, false);
            store_ireg_to_ir1_bd(ir2_src, dest, false);
            return true;
        } else if (ir1_opnd_is_mmx_bd(dest) && ir1_opnd_is_gpr_bd(src)) {
            lsassert(ir1_opnd_size_bd(src) == 64);
            /* transfer to mmx mode */
            transfer_to_mmx_mode_bd();
            IR2_OPND ir2_src =
    ...
    if (ir1_opnd_is_xmm_bd(ir1_get_opnd_bd(pir1, 0))) { /* dest xmm */
        IR2_OPND dest_lo = load_freg_from_ir1_1_bd(ir1_get_opnd_bd(pir1, 0), false, false);
        load_freg_from_ir1_2_bd(dest_lo, ir1_get_opnd_bd(pir1, 0) + 1, 0);
        store_ireg_to_ir1_bd(zero_ir2_opnd, ir1_get_opnd_bd(pir1, 0), true);
    } else if (ir1_opnd_is_mmx_bd(ir1_get_opnd_bd(pir1, 0))) { /* dest mmx */
        /* transfer to mmx mode */
        transfer_to_mmx_mode_bd();
        IR2_OPND dest_opnd = load_freg_from_ir1_1_bd(ir1_get_opnd_bd(pir1, 0), false, false);
        load_freg_from_ir1_2_bd(dest_opnd, ir1_get_opnd_bd(pir1, 0) + 1, IS_INTEGER);
    } else { /* dest mem */
        IR2_OPND source_opnd =
            load_freg_from_ir1_1_bd(ir1_get_opnd_bd(pir1, 0) + 1, false, false);
        store_freg_to_ir1_bd(source_opnd, ir1_get_opnd_bd(pir1, 0), false, false);
    }
    return true;
}

bool translate_movd_bd(IR1_INST *pir1)
{
    IR1_OPND_BD *dest = ir1_get_opnd_bd(pir1, 0);
    IR1_OPND_BD *src = ir1_get_opnd_bd(pir1, 1);
    if (ir1_opnd_is_xmm_bd(dest) && ir1_opnd_is_mem_bd(src)) {
        IR2_OPND temp = load_freg_from_ir1_1_bd(src, false, false);
        if (option_enable_lasx) {
            la_xvpickve_w(ra_alloc_xmm(ir1_opnd_base_reg_num_bd(dest)), temp, 0);
        } else {
            la_vandi_b(ra_alloc_xmm(ir1_opnd_base_reg_num_bd(dest)),
                       ra_alloc_xmm(ir1_opnd_base_reg_num_bd(dest)), 0);
            la_vextrins_w(ra_alloc_xmm(ir1_opnd_base_reg_num_bd(dest)), temp, 0);
        }
        return true;
    }
    if (ir1_opnd_is_xmm_bd(dest) && ir1_opnd_is_gpr_bd(src)) {
        if (option_enable_lasx) {
            ...
            la_movgr2fr_w(temp, ra_alloc_gpr(ir1_opnd_base_reg_num_bd(src)));
            la_xvpickve_w(ra_alloc_xmm(ir1_opnd_base_reg_num_bd(dest)), temp, 0);
        } else {
            la_vandi_b(ra_alloc_xmm(ir1_opnd_base_reg_num_bd(dest)),
                       ra_alloc_xmm(ir1_opnd_base_reg_num_bd(dest)), 0);
            la_vinsgr2vr_w(ra_alloc_xmm(ir1_opnd_base_reg_num_bd(dest)),
                          ra_alloc_gpr(ir1_opnd_base_reg_num_bd(src)), 0);
        }
        return true;
    }
    if (ir1_opnd_is_mem_bd(dest) && ir1_opnd_is_xmm_bd(src)) {
        store_freg_to_ir1_bd(ra_alloc_xmm(ir1_opnd_base_reg_num_bd(src)), dest,
                          false, false);
        return true;
    }
    if (ir1_opnd_is_gpr_bd(dest) && ir1_opnd_is_xmm_bd(src)) {
        la_movfr2gr_s(ra_alloc_gpr(ir1_opnd_base_reg_num_bd(dest)),
                      ra_alloc_xmm(ir1_opnd_base_reg_num_bd(src)));
#ifdef TARGET_X86_64
        if (!GHBR_ON_BD(pir1) && CODEIS64) {
            IR2_OPND ir2_dest = ra_alloc_gpr(ir1_opnd_base_reg_num_bd(dest));
    ...
    if (ir1_opnd_is_gpr_bd(ir1_get_opnd_bd(pir1, 0)) || ir1_opnd_is_mem_bd(ir1_get_opnd_bd(pir1, 0))) {
        lsassert(ir1_opnd_is_mmx_bd(ir1_get_opnd_bd(pir1, 0) + 1) ||
                 ir1_opnd_is_xmm_bd(ir1_get_opnd_bd(pir1, 0) + 1));

        if (ir1_opnd_is_mmx_bd(ir1_get_opnd_bd(pir1, 0) + 1)) {
            /* transfer to mmx mode */
            transfer_to_mmx_mode_bd();
        }

        IR2_OPND src =
            load_ireg_from_ir1_bd(ir1_get_opnd_bd(pir1, 0) + 1, UNKNOWN_EXTENSION, false);
        store_ireg_to_ir1_bd(src, ir1_get_opnd_bd(pir1, 0), false);
    } else if (ir1_opnd_is_gpr_bd(ir1_get_opnd_bd(pir1, 0) + 1) || ir1_opnd_is_mem_bd(ir1_get_opnd_bd(pir1, 0) + 1)) {
        lsassert(ir1_opnd_is_mmx_bd(ir1_get_opnd_bd(pir1, 0)) ||
                 ir1_opnd_is_xmm_bd(ir1_get_opnd_bd(pir1, 0)));

        if (ir1_opnd_is_mmx_bd(ir1_get_opnd_bd(pir1, 0))) {
            /* transfer to mmx mode */
            transfer_to_mmx_mode_bd();
        }

        IR2_OPND src =
            load_ireg_from_ir1_bd(ir1_get_opnd_bd(pir1, 0) + 1, ZERO_EXTENSION, false);
        store_ireg_to_ir1_bd(src, ir1_get_opnd_bd(pir1, 0), false);
...
bool translate_pushaw_bd(IR1_INST *pir1) {
    IR2_OPND esp_opnd = ra_alloc_gpr(esp_index);
    ...
    IR1_OPND_BD mem_ir1_opnd;
    for (int i = 0; i < 8; i++) {
        ir1_opnd_build_mem_bd(&mem_ir1_opnd, esp_decrement << 3, NDR_ESP, 32,
                           -esp_decrement * (i + 1));
        IR2_OPND src_opnd = ra_alloc_gpr(i);
        store_ireg_to_ir1_bd(src_opnd, &mem_ir1_opnd, false);
    }
...
bool translate_pushal_bd(IR1_INST *pir1) {
    IR2_OPND esp_opnd = ra_alloc_gpr(esp_index);
...
bool translate_popaw_bd(IR1_INST *pir1) {
    IR2_OPND esp_opnd = ra_alloc_gpr(esp_index);
    ...
    for (int i = 7; i >= 0; i--) {
        ...
        ir1_opnd_build_mem_bd(&mem_ir1_opnd, esp_increment << 3, NDR_ESP, 32,
                           esp_increment * (7 - i));
        load_ireg_from_ir1_2_bd(dst_opnd, &mem_ir1_opnd, UNKNOWN_EXTENSION, false);
    }
...
bool translate_popal_bd(IR1_INST *pir1) {
    IR2_OPND esp_opnd = ra_alloc_gpr(esp_index);
...
* }
*/
bool translate_popcnt_bd(IR1_INST *pir1) {
    IR2_OPND src_opnd =
        load_ireg_from_ir1_bd(ir1_get_opnd_bd(pir1, 1), ZERO_EXTENSION, false);
    IR2_OPND count = ra_alloc_itemp();
    IR2_OPND temp = ra_alloc_itemp();
    IR2_OPND src_temp = ra_alloc_itemp();
    IR2_OPND pop_count_ir2_opnd = ra_alloc_itemp();
    int pop_count = ir1_opnd_size_bd(ir1_get_opnd_bd(pir1, 0));
    IR2_OPND label_exit = ra_alloc_label();
...
/* label_exit: */
    la_label(label_exit);
    store_ireg_to_ir1_bd(count, ir1_get_opnd_bd(pir1, 0), false);
    ra_free_temp(count);
    return true;
}
