...
{
    lsassert(ir1_opnd_is_gpr_bd(opnd1));
    int gpr_num = ir1_opnd_base_reg_num_bd(opnd1);
    ...
    /* 3. 16 bits gpr uses AND operation when zero extended */
        ...
        IR2_OPND gpr_opnd = ra_alloc_gpr(ir1_opnd_base_reg_num_bd(opnd1));
    ...
    /* 4. 8 bits gpr high */
    else if (ir1_opnd_is_8h_bd(opnd1)) {
        IR2_OPND gpr_opnd = ra_alloc_gpr(ir1_opnd_base_reg_num_bd(opnd1));
        IR2_OPND ret_opnd = ra_alloc_itemp_internal();
...
*/
static void load_ireg_from_ir1_imm_bd(IR2_OPND opnd2, IR1_OPND_BD *opnd1,
                                   EXTENSION_MODE em)
{
    lsassert(ir2_opnd_is_ireg(&opnd2));
    lsassert(ir1_opnd_is_imm_bd(opnd1));
    if (em == ZERO_EXTENSION) {
        ulongx value = ir1_opnd_uimm_bd(opnd1);
        li_d(opnd2, value);
    } else {
        longx value = ir1_opnd_simm_bd(opnd1);
        li_d(opnd2, value);
    }
}

static void load_ireg_from_ir1_const_bd(IR2_OPND opnd2, IR1_OPND_BD *opnd1,
                                   EXTENSION_MODE em)
{
    lsassert(ir2_opnd_is_ireg(&opnd2));
    lsassert(ir1_opnd_is_const_bd(opnd1));
    uint32_t value = ir1_opnd_const_bd(opnd1);
    if (em == ZERO_EXTENSION) {
        li_d(opnd2, (ulongx)value);
    } else {
        li_d(opnd2, (longx)value);
    }
}
...
{
    lsassert(ir2_opnd_is_ireg(&opnd2));
    lsassert(ir1_opnd_is_mem_bd(opnd1));
    ...
    if ((mem_imm & 0xffff0000) == 0xdead0000) {
        ...
        if (ir1_opnd_size_bd(opnd1) == 32) {
            if (em == ZERO_EXTENSION) {
                la_ldx_wu(opnd2, base, index);
            } else {
                la_ldx_w(opnd2, base, index);
            }
        } else if (ir1_opnd_size_bd(opnd1) == 64 || ir1_opnd_size_bd(opnd1) == 128) {
            la_ldx_d(opnd2, base, index);
        } else if (ir1_opnd_size_bd(opnd1) == 8) {
            if (em == ZERO_EXTENSION)
    	        la_ldx_bu(opnd2, base, index);
    	    else
    	        la_ldx_b(opnd2, base, index);
        } else if (ir1_opnd_size_bd(opnd1) == 16) {
            if (em == ZERO_EXTENSION)
...
@return the ir2 register operand. it can be temp register or a mapped register
*/
static void load_ireg_from_ir1_gpr_bd(IR2_OPND opnd2, IR1_OPND_BD *opnd1,
                                   EXTENSION_MODE em)
{
    lsassert(ir2_opnd_is_ireg(&opnd2));
    lsassert(ir1_opnd_is_gpr_bd(opnd1));
    ...
    /* 2. 64 bits gpr needs no extension */
    else if (ir1_opnd_size_bd(opnd1) == 64) {
        la_mov64(opnd2, gpr_opnd);
        return;
    }
...
static void load_ireg_from_ir1_mmx_bd(IR2_OPND opnd2, IR1_OPND_BD *opnd1,
                                   EXTENSION_MODE em)
{
    IR2_OPND mmx_opnd = ra_alloc_mmx(ir1_opnd_base_reg_num_bd(opnd1));
...
IR2_OPND load_ireg_from_ir1_bd(IR1_OPND_BD *opnd1, EXTENSION_MODE em, bool is_xmm_hi)
{
    ...
    case ND_OP_IMM: {
        if (ir1_opnd_simm_bd(opnd1) == 0)
            ret_opnd = zero_ir2_opnd;
        else {
            ret_opnd = ra_alloc_itemp_internal();
            load_ireg_from_ir1_imm_bd(ret_opnd, opnd1, em);
        }
        break;
    }
    ...
    case ND_OP_REG:  {
        if(ir1_opnd_is_gpr_bd(opnd1)){
            ret_opnd = convert_gpr_opnd_bd(opnd1, em);
            /* Now it is OK for UNKNOWN_EXTENSION */
            break;
        }
        else if(ir1_opnd_is_mmx_bd(opnd1)){
            ret_opnd = ra_alloc_itemp_internal();
            load_ireg_from_ir1_mmx_bd(ret_opnd, opnd1, em);
            break;
        }
        else if(ir1_opnd_is_ymm_bd(opnd1)){
            lsassert(0);
        }
        else if(ir1_opnd_is_seg_bd(opnd1)){
            ret_opnd = ra_alloc_itemp_internal();
            int seg_num = ir1_opnd_base_reg_num_bd(opnd1);
    ...
    case ND_OP_MEM: {
        ret_opnd = ra_alloc_itemp_internal();
        load_ireg_from_ir1_mem_bd(ret_opnd, opnd1, em, is_xmm_hi);
        break;
    }
    case ND_OP_CONST: {
        if (ir1_opnd_const_bd(opnd1) == 0)
            ret_opnd = zero_ir2_opnd;
        else {
            ret_opnd = ra_alloc_itemp_internal();
            load_ireg_from_ir1_const_bd(ret_opnd, opnd1, em);
        }
        break;
    }
    default:
                          ...
                          bool is_xmm_hi)
{
    ...
    switch (ir1_opnd_type_bd(opnd1)) {
    case ND_OP_IMM:
        load_ireg_from_ir1_imm_bd(opnd2, opnd1, em);
        break;
    case ND_OP_REG:  {
        if(ir1_opnd_is_gpr_bd(opnd1)){
            load_ireg_from_ir1_gpr_bd(opnd2, opnd1, em);
            break;
        }
        else if(ir1_opnd_is_mmx_bd(opnd1)){
            load_ireg_from_ir1_mmx_bd(opnd2, opnd1, em);
            break;
        }
        else if(ir1_opnd_is_ymm_bd(opnd1)){
            lsassert(0);
    ...
    case ND_OP_MEM:  {
        load_ireg_from_ir1_mem_bd(opnd2, opnd1, em, is_xmm_hi);
        break;
    }
    case ND_OP_CONST: {
        load_ireg_from_ir1_const_bd(opnd2, opnd1, em);
        break;
    }
    default:
        lsassert(0);
...
static void store_ireg_to_ir1_gpr_bd(IR2_OPND opnd2, IR1_OPND_BD *opnd1)
{
    lsassert(ir2_opnd_is_ireg(&opnd2));
    lsassert(ir1_opnd_is_gpr_bd(opnd1) && ir2_opnd_is_ireg(&opnd2));
    int gpr_num = ir1_opnd_base_reg_num_bd(opnd1);
    IR2_OPND gpr_opnd = ra_alloc_gpr(gpr_num);
...
static void store_ireg_to_ir1_mem_bd(IR2_OPND value_opnd, IR1_OPND_BD *opnd1,
                                  bool is_xmm_hi)
{
...
void store_ireg_to_ir1_bd(IR2_OPND opnd2, IR1_OPND_BD *opnd1, bool is_xmm_hi)
{
    ...
    case ND_OP_REG:  {
        if(ir1_opnd_is_gpr_bd(opnd1)){
            store_ireg_to_ir1_gpr_bd(opnd2, opnd1);
            return;
        }
        else if(ir1_opnd_is_mmx_bd(opnd1)){
            IR2_OPND mmx_opnd = ra_alloc_mmx(ir1_opnd_base_reg_num_bd(opnd1));
            ...
            store_ireg_to_ir1_seg_bd(opnd2, opnd1);
            return;
        }
    ...
    case ND_OP_MEM:
        store_ireg_to_ir1_mem_bd(opnd2, opnd1, is_xmm_hi);
        return;
    default:
        lsassert(0);
        return;
...
@return the ir2 register operand
*/
static void load_freg_from_ir1_mem_bd(IR2_OPND opnd2, IR1_OPND_BD *opnd1,
                                   bool is_xmm_hi, uint32 options)
{
    ...
    if (ir1_opnd_size_bd(opnd1) == 32) {
        ...
        load_64_bit_freg_from_ir1_80_bit_mem_bd(opnd2, mem_opnd, mem_imm);
    }
}
...
static void load_freg_from_ir1_fpr_bd(IR2_OPND opnd2, IR1_OPND_BD *opnd1)
{
    lsassert(ir2_opnd_is_freg(&opnd2));
    IR2_OPND value_opnd = ra_alloc_st(ir1_opnd_base_reg_num_bd(opnd1));
    if (!ir2_opnd_cmp(&opnd2, &value_opnd))
        la_fmov_d(opnd2, value_opnd);
}

static void load_freg_from_ir1_mmx_bd(IR2_OPND opnd2, IR1_OPND_BD *opnd1)
{
    lsassert(ir2_opnd_is_freg(&opnd2));
    lsassert(ir1_opnd_is_mmx_bd(opnd1));
    IR2_OPND mmx_opnd = ra_alloc_mmx(ir1_opnd_base_reg_num_bd(opnd1));
...
IR2_OPND load_freg_from_ir1_1_bd(IR1_OPND_BD *opnd1, bool is_xmm_hi, uint32_t options)
{
    ...
    switch (ir1_opnd_type_bd(opnd1)) {
    case ND_OP_REG: {
        if (ir1_opnd_is_fpr_bd(opnd1)){
            return ra_alloc_st(ir1_opnd_base_reg_num_bd(opnd1));

        }
        else if(ir1_opnd_is_mmx_bd(opnd1)){
            return ra_alloc_mmx(ir1_opnd_base_reg_num_bd(opnd1));
    ...
    case ND_OP_MEM: {
        ...
        load_freg_from_ir1_mem_bd(ret_opnd, opnd1, is_xmm_hi, options);
        return ret_opnd;
    }
    default:
        lsassert(0);
...
void load_freg_from_ir1_2_bd(IR2_OPND opnd2, IR1_OPND_BD *opnd1, uint32_t options)
{
    ...
    case ND_OP_REG: {
        if (ir1_opnd_is_fpr_bd(opnd1)){
            load_freg_from_ir1_fpr_bd(opnd2, opnd1);
            return;
        }
        else if(ir1_opnd_is_mmx_bd(opnd1)){
            load_freg_from_ir1_mmx_bd(opnd2, opnd1);
            return;
        }
        else if(ir1_opnd_is_ymm_bd(opnd1)){
            lsassert(0);
        }
        lsassertm(0,"REG:%d\n", opnd1->Info.Register.Reg);
        return;
    }

    case ND_OP_MEM: {
        load_freg_from_ir1_mem_bd(opnd2, opnd1, is_xmm_hi,
                                ...
                                IR1_OPND_BD *opnd1, bool is_xmm_hi)
{
    ...
    case ND_OP_MEM:  {
        ...
        load_ireg_from_ir1_mem_bd(itemp0, opnd1, ZERO_EXTENSION, is_xmm_hi);
...
static void store_freg_to_ir1_mem_bd(IR2_OPND opnd2, IR1_OPND_BD *opnd1,
                                  bool is_xmm_hi, bool is_convert)
{
    ...
    if (ir1_opnd_size_bd(opnd1) == 32) {
        ...
        store_64_bit_freg_to_ir1_80_bit_mem_bd(opnd2, mem_opnd, mem_imm);
    }
}

static void store_freg_to_ir1_gpr_bd(IR2_OPND opnd2, IR1_OPND_BD *opnd1)
{
    lsassert(ir2_opnd_is_freg(&opnd2));
    IR2_OPND target_opnd = ra_alloc_gpr(ir1_opnd_base_reg_num_bd(opnd1));
    if (ir1_opnd_size_bd(opnd1) == 32) {
...
static void store_freg_to_ir1_fpr_bd(IR2_OPND opnd2, IR1_OPND_BD *opnd1)
{
    lsassert(ir2_opnd_is_freg(&opnd2));
    IR2_OPND target_opnd = ra_alloc_st(ir1_opnd_base_reg_num_bd(opnd1));
...
*/
static void store_freg_to_ir1_mmx_bd(IR2_OPND opnd2, IR1_OPND_BD *opnd1)
{
    lsassert(ir2_opnd_is_ireg(&opnd2));
    lsassert(ir1_opnd_is_mmx_bd(opnd1));
                       ...
                       bool is_convert)
{
    ...
    case ND_OP_REG:  {
        if(ir1_opnd_is_fpr_bd(opnd1)){
            store_freg_to_ir1_fpr_bd(opnd2, opnd1);
            return;
        }
        else if(ir1_opnd_is_mmx_bd(opnd1)){
            store_freg_to_ir1_mmx_bd(opnd2, opnd1);
            return;
        }
        else if(ir1_opnd_is_ymm_bd(opnd1)){
            lsassert(0);
        ...
        if(ir1_opnd_is_gpr_bd(opnd1)){
            store_freg_to_ir1_gpr_bd(opnd2, opnd1);
            return;
        }
        lsassertm(0, "REG:%d\n", opnd1->Info.Register.Reg);
        return;
    ...
    case ND_OP_MEM: {
        store_freg_to_ir1_mem_bd(opnd2, opnd1, is_xmm_hi, is_convert);
        return;
    }
    default:
        lsassert(0);
...
void load_freg128_from_ir1_mem_bd(IR2_OPND opnd2, IR1_OPND_BD *opnd1){
    ...
    IR2_OPND mem_opnd = convert_mem_bd(opnd1, &little_disp);
...
IR2_OPND load_freg128_from_ir1_bd(IR1_OPND_BD *opnd1){
    lsassert(ir1_opnd_is_xmm_bd(opnd1) || ir1_opnd_is_mem_bd(opnd1));

    if (ir1_opnd_is_xmm_bd(opnd1)) {
        return ra_alloc_xmm(ir1_opnd_base_reg_num_bd(opnd1));
    } else if (ir1_opnd_is_mem_bd(opnd1)) {
        IR2_OPND ret_opnd = ra_alloc_ftemp();
        if (ir1_opnd_size_bd(opnd1) == 128) {
            load_freg128_from_ir1_mem_bd(ret_opnd, opnd1);
        } else {
            load_freg_from_ir1_mem_bd(ret_opnd, opnd1, 0, IS_INTEGER);
        }
        return ret_opnd;
    }
...
IR2_OPND load_freg256_from_ir1_bd(IR1_OPND_BD * opnd1) {
    if (ir1_opnd_is_ymm_bd(opnd1)) {
        return ra_alloc_xmm(ir1_opnd_base_reg_num_bd(opnd1));
    } else if (ir1_opnd_is_mem_bd(opnd1) || ir1_opnd_is_xmm_bd(opnd1)) {
        IR2_OPND ret_opnd;
        if (ir1_opnd_size_bd(opnd1) == 256) {
            ret_opnd = ra_alloc_ftemp();
            load_freg256_from_ir1_mem_bd(ret_opnd, opnd1);
        } else {
            ret_opnd = load_freg128_from_ir1_bd(opnd1);
        }
        return ret_opnd;
    }
    g_assert_not_reached();
