...
static void print_debug_info(void)
{
    ...
    target_ulong curr_pc __attribute__((unused))  = ir1_addr_bd(pir1);
...
*/
__attribute__((unused))
static IR2_OPND convert_mem_helper(IR1_OPND_BD *opnd1, IR2_OPND *arg_dest_op,
...
#define BIT_BASE        (1 << 2)
    ...
    has_seg = ir1_opnd_has_seg_bd(opnd1);
   /*
    * bitmap:
    *           base    index   offset
    *   case1    0        0       1
    ...
    *   case7    1        1       1
    */
    has_index = ir1_opnd_has_index_bd(opnd1);
    has_base = ir1_opnd_has_base_bd(opnd1);
    ...
    if (has_base) {
        base = ir1_opnd_base_reg_num_bd(opnd1);
    }
    if (has_index) {
        index = ir1_opnd_index_reg_num_bd(opnd1);
        scale = ir1_opnd_scale_bd(opnd1);
    }
    ...
    if (has_base) {
        bitmap |= BIT_BASE;
        base_op = ra_alloc_gpr(ir1_opnd_base_reg_num_bd(opnd1));
    }
    if (has_index) {
        bitmap |= BIT_INDEX;
        index_op = ra_alloc_gpr(ir1_opnd_index_reg_num_bd(opnd1));
    }
#ifdef CONFIG_LATX_IMM_REG
...
imm_cache_exit:
    ...
    case 3:
        if (ir2_opnd_is_itemp(&dest_op)) {
            ...
            index_add_base(dest_op, dest_op,
                    index_op, ir1_opnd_scale_bd(opnd1));
        } else {
            temp_op = ra_alloc_itemp();
    ...
    /* base + index * scale */
    case 6:
        index_add_base(dest_op, base_op, index_op, ir1_opnd_scale_bd(opnd1));
        break;
    /* base + index * scale + offset */
    case 7:
        index_add_base(dest_op, base_op, index_op, ir1_opnd_scale_bd(opnd1));
...
skip:
    ...
    if (has_seg) {
        ...
        int index = ir1_opnd_get_seg_index_bd(opnd1);
       ...
       if (bitmap) {
        ...
        lsassert(ir1_opnd_simm_bd(opnd1));
#else
        /* fprintf(stderr, "maybe have bug in convert_mem_helper()\n"); */
#endif
    }

    addr_size = ir1_addr_size_bd(pir1);
