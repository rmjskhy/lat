...
bool translate_xor_bd(IR1_INST *pir1)
{
    ...
    if (is_lock) {
        return translate_lock_xor_bd(pir1);
    }
#endif
    ...
    opt_imm = ir1_opnd_is_s2uimm12_bd(opnd1);
    eflags_calc = ir1_need_calculate_any_flag_bd(pir1);
    ...
    if (ir1_opnd_is_same_reg_bd(opnd0, opnd1) && ir1_opnd_size_bd(opnd0) >= 32) {
        la_mov64(dest, zero_ir2_opnd);
        generate_eflag_calculation_bd(dest, dest, dest, pir1, true);
        return true;
    }

    generate_eflag_calculation_bd(dest, src0, src1, pir1, true);
    if (opt_imm) {
        la_xori(dest, src0, ir1_opnd_s2uimm_bd(opnd1));
    } else {
...
bool translate_and_bd(IR1_INST *pir1)
{
    IR1_OPND_BD *opnd0 = ir1_get_opnd_bd(pir1, 0);
    IR1_OPND_BD *opnd1 = ir1_get_opnd_bd(pir1, 1);
    CPUArchState* env = (CPUArchState*)(lsenv->cpu_state);
...
#ifdef CONFIG_LATX_LLSC
    if (is_lock) {
        return translate_lock_and_bd(pir1);
    }
#endif
    ...
    if (ir1_opnd_is_imm_bd(opnd1)) {
        special_mask = imm_mask(ir1_opnd_simm_bd(opnd1), &low_mask, &mask_pos);
    ...
    if (special_mask) {
        ...
        la_andi(dest, src0, ir1_opnd_s2uimm_bd(opnd1));
    } else {
    ...
    if (ir1_opnd_is_gpr_bd(opnd0)) {
        ...
        if (!GHBR_ON_BD(pir1) && CODEIS64 && opnd0_size == 32 && !low_mask) {
            la_mov32_zx(dest, dest);
        } else
#endif
        /* r16/r8 */
...
bool translate_test_bd(IR1_INST *pir1)
{
    IR2_OPND src_opnd_0 =
    ...
    generate_eflag_calculation_bd(src_opnd_1, src_opnd_0, src_opnd_1, pir1, true);
    return true;
}

bool translate_or_bd(IR1_INST *pir1)
{
    IR1_OPND_BD *opnd0 = ir1_get_opnd_bd(pir1, 0);
    IR1_OPND_BD *opnd1 = ir1_get_opnd_bd(pir1, 1);
    CPUArchState* env = (CPUArchState*)(lsenv->cpu_state);
...
#ifdef CONFIG_LATX_LLSC
    if (is_lock) {
        return translate_lock_or_bd(pir1);
    }
#endif
    ...
    /* calculate */
    if (opt_imm) {
        la_ori(dest, src0, ir1_opnd_s2uimm_bd(opnd1));
    } else {
...
bool translate_not_bd(IR1_INST *pir1)
{
    IR1_OPND_BD *opnd0 = ir1_get_opnd_bd(pir1, 0);
    CPUArchState* env = (CPUArchState*)(lsenv->cpu_state);
    CPUState *cpu = env_cpu(env);
...
#ifdef CONFIG_LATX_LLSC
    if (is_lock) {
        return translate_lock_not_bd(pir1);
    }
#endif
 ...
 * @return false
 */
bool translate_shl_bd(IR1_INST *pir1)
{
    IR1_OPND_BD *opnd1 = ir1_get_opnd_bd(pir1, 0);
    IR1_OPND_BD *opnd2 = ir1_get_opnd_bd(pir1, 1);

    int opnd_size = ir1_opnd_size_bd(opnd1);
    bool dest_is_gpr = ir1_opnd_is_gpr_bd(opnd1);
    bool directly = dest_is_gpr && (opnd_size >= 32);

    uint32 mask = (ir1_opnd_size_bd(opnd1) == 64) ? 63 : 31;
    ...
    if (directly) {
        ...
        dest = src = ra_alloc_gpr(ir1_opnd_base_reg_num_bd(opnd1));
    ...
    if (ir1_opnd_is_imm_bd(opnd2)) {
         ...
         */
        if (shift || directly) {
            store_ireg_to_ir1_bd(dest, opnd1, false);
        }
        ...
        IR2_OPND original_count =
            load_ireg_from_ir1_bd(opnd2, UNKNOWN_EXTENSION, false);
        ...
        generate_eflag_calculation_bd(dest, src, count, pir1, false);
 ...
 * @return false
 */
bool translate_shr_bd(IR1_INST *pir1)
{
    IR1_OPND_BD *opnd1 = ir1_get_opnd_bd(pir1, 0);
    IR1_OPND_BD *opnd2 = ir1_get_opnd_bd(pir1, 1);
    ...
    if (directly) {
        ...
        src = load_ireg_from_ir1_bd(opnd1, ZERO_EXTENSION, false);
        dest = ra_alloc_itemp();
        shift_inst = la_srl_d;
...
bool translate_sal_bd(IR1_INST *pir1)
{
    return translate_shl_bd(pir1);
}
 ...
 * @return false
 */
bool translate_sar_bd(IR1_INST *pir1)
{
    IR1_OPND_BD *opnd1 = ir1_get_opnd_bd(pir1, 0);
    IR1_OPND_BD *opnd2 = ir1_get_opnd_bd(pir1, 1);
    ...
    if (directly) {
        ...
        src = load_ireg_from_ir1_bd(opnd1, SIGN_EXTENSION, false);
        dest = ra_alloc_itemp();
        shift_inst = la_sra_d;
...
bool translate_rol_bd(IR1_INST *pir1)
{
    IR1_OPND_BD *opnd0 = ir1_get_opnd_bd(pir1, 0);
    int dest_size = ir1_opnd_size_bd(opnd0);

    /* get real count */
    IR2_OPND label_exit = ra_alloc_label();

    IR2_OPND original_count;
    if (ir1_get_opnd_num_bd(pir1) == 1) {
    ...
    if (CODEIS64) {
        ...
        if (!GHBR_ON_BD(pir1) && ir1_opnd_size_bd(ir1_get_opnd_bd(pir1, 0)) == 32
        && ir1_opnd_is_gpr_bd(ir1_get_opnd_bd(pir1, 0))) {
            /* clean the target*/
            IR2_OPND gpr_opnd = ra_alloc_gpr(ir1_opnd_base_reg_num_bd(ir1_get_opnd_bd(pir1, 0)));
    ...
    IR2_OPND dest = load_ireg_from_ir1_bd(opnd0, ZERO_EXTENSION, false);
    IR2_OPND tmp_dest = ra_alloc_itemp();

    if (ir1_need_calculate_any_flag_bd(pir1)) {
        if (ir1_opnd_size_bd(opnd0) == 8) {
        ...
        } else if (ir1_opnd_size_bd(opnd0) == 64) {
...
#endif
    store_ireg_to_ir1_bd(tmp_dest, ir1_get_opnd_bd(pir1, 0), false);
    la_label(label_exit);
    ra_free_temp(tmp);
...
bool translate_ror_bd(IR1_INST *pir1)
{
    IR1_OPND_BD *opnd0 = ir1_get_opnd_bd(pir1, 0);
    int dest_size = ir1_opnd_size_bd(opnd0);
...
#else
    if (CODEIS64) {
        if (ir1_opnd_size_bd(ir1_get_opnd_bd(pir1, 0)) == 64) {
            la_andi(count, original_count, 0x3f);
        } else {
            la_andi(count, original_count, 0x1f);
        }
    ...
    if (ir1_need_calculate_any_flag_bd(pir1)) {
        ...
        } else if (ir1_opnd_size_bd(ir1_get_opnd_bd(pir1, 0)) == 64) {
...
bool translate_rorx_bd(IR1_INST *pir1)
{
    IR1_OPND_BD *opnd0 = ir1_get_opnd_bd(pir1, 0);
    IR2_OPND dest = ra_alloc_gpr(ir1_opnd_base_reg_num_bd(opnd0));
    IR2_OPND src = load_ireg_from_ir1_bd(ir1_get_opnd_bd(pir1, 1), ZERO_EXTENSION, false);
    IR2_OPND imm = load_ireg_from_ir1_bd(ir1_get_opnd_bd(pir1, 2), ZERO_EXTENSION, false);
    int dest_size = ir1_opnd_size_bd(opnd0);
    IR2_OPND count = ra_alloc_itemp();
    IR2_OPND tmp = ra_alloc_itemp();
    if (dest_size == 32) {
...
bool translate_rcl_bd(IR1_INST *pir1)
{
    IR1_OPND_BD *opnd0 = ir1_get_opnd_bd(pir1, 0);
    int dest_size = ir1_opnd_size_bd(opnd0);
    ...
    if (CODEIS64) {
        ...
        la_andi(count, original_count, mask);
        if (!GHBR_ON_BD(pir1) && dest_size == 32 && ir1_opnd_is_gpr_bd(ir1_get_opnd_bd(pir1, 0))) {
            /* clean the target*/
            IR2_OPND gpr_opnd =
                ra_alloc_gpr(ir1_opnd_base_reg_num_bd(ir1_get_opnd_bd(pir1, 0)));
            la_mov32_zx(gpr_opnd, gpr_opnd);
            la_beq(count, zero_ir2_opnd, label_exit);
        } else {
            la_beq(count, zero_ir2_opnd, label_exit);
    ...
    get_eflag_condition_bd(&tmp_dest, pir1);
    ...
    la_or(tmp_dest, tmp_dest, tmp);
    la_bstrins_d(tmp_dest, zero_ir2_opnd, 63, 32);
    generate_eflag_calculation_bd(tmp_dest, dest, count, pir1, true);
    store_ireg_to_ir1_bd(tmp_dest, ir1_get_opnd_bd(pir1, 0), false);
    ra_free_temp(tmp);
    ra_free_temp(tmp_dest);
#else
    if (CODEIS64) {
        ...
        get_eflag_condition_bd(&cf, pir1);
        if (dest_size <= 32) {
    ...
    /* label_finish: */
        ...
        generate_eflag_calculation_bd(high_dest, dest, count, pir1, true);
        /* we can't move the dest directly */
        /* because eflag_calculation need the resource data */
        store_ireg_to_ir1_bd(high_dest, ir1_get_opnd_bd(pir1, 0), false);
...
bool translate_rcr_bd(IR1_INST *pir1)
{
    IR1_OPND_BD *opnd0 = ir1_get_opnd_bd(pir1, 0);
    int dest_size = ir1_opnd_size_bd(opnd0);
    ...
    /* label_finish: */
        la_label(label_finish);
        generate_eflag_calculation_bd(low_dest, dest, count, pir1, true);
        ra_free_temp(count);
        store_ireg_to_ir1_bd(low_dest, ir1_get_opnd_bd(pir1, 0), false);
        ra_free_temp(low_dest);
    } else {
        IR2_OPND tmp_dest = ra_alloc_itemp();
...
{
    IR2_OPND count_opnd =
        load_ireg_from_ir1_bd(ir1_get_opnd_bd(pir1, 0) + 2, ZERO_EXTENSION, false);
...
#ifdef TARGET_X86_64
    if (!GHBR_ON_BD(pir1) && CODEIS64 && ir1_opnd_is_gpr_bd(ir1_get_opnd_bd(pir1, 0)) &&
        opnd0_size == 32) {
        /* clean the target */
        IR2_OPND dest_opnd =
                ra_alloc_gpr(ir1_opnd_base_reg_num_bd(ir1_get_opnd_bd(pir1, 0)));
    ...
    load_ireg_from_ir1_2_bd(dest_opnd, ir1_get_opnd_bd(pir1, 0), ZERO_EXTENSION, false);
    load_ireg_from_ir1_2_bd(src_opnd, ir1_get_opnd_bd(pir1, 1), ZERO_EXTENSION, false);
    ...
    generate_eflag_calculation_bd(final_dest, dest_opnd, count, pir1, true);
    ra_free_temp(dest_opnd);
    ra_free_temp(count);

    store_ireg_to_ir1_bd(final_dest, ir1_get_opnd_bd(pir1, 0), false);
...
static bool translate_shrd_imm(IR1_INST *pir1)
{
    lsassert(ir1_opnd_is_imm_bd(ir1_get_opnd_bd(pir1, 0) + 2));
    int opnd0_size = ir1_opnd_size_bd(ir1_get_opnd_bd(pir1, 0));
    int32 mask = (opnd0_size == 64) ? 0x3f : 0x1f;
    int count = ir1_opnd_simm_bd(ir1_get_opnd_bd(pir1, 2)) & mask;
    ...
    generate_eflag_calculation_bd(final_dest, dest_opnd, count_opnd, pir1, true);
...
bool translate_shrd_bd(IR1_INST *pir1)
{
    if (ir1_opnd_is_imm_bd(ir1_get_opnd_bd(pir1, 0) + 2))
...
bool translate_shld_bd(IR1_INST *pir1)
{
    if (ir1_opnd_is_imm_bd(ir1_get_opnd_bd(pir1, 0) + 2))
...
bool translate_bswap_bd(IR1_INST *pir1)
{
    IR2_OPND bswap_opnd;
    IR1_OPND_BD *ir1_opnd = ir1_get_opnd_bd(pir1, 0);
    int opnd_size = ir1_opnd_size_bd(ir1_opnd);

    if(opnd_size == 16){
        set_ir1_opnd_size_bd(ir1_opnd, (32 >> 3));
        switch (ir1_opnd_base_reg_num_bd(ir1_opnd)) {
                case NDR_AX:
            set_ir1_opnd_base_reg_bd(ir1_opnd, NDR_EAX);
            break;
        case NDR_BX:
            set_ir1_opnd_base_reg_bd(ir1_opnd, NDR_EBX);
            break;
        case NDR_CX:
            set_ir1_opnd_base_reg_bd(ir1_opnd, NDR_ECX);
            break;
        case NDR_DX:
            set_ir1_opnd_base_reg_bd(ir1_opnd, NDR_EDX);
            break;
        case NDR_SP:
            set_ir1_opnd_base_reg_bd(ir1_opnd, NDR_ESP);
            break;
        case NDR_BP:
            set_ir1_opnd_base_reg_bd(ir1_opnd, NDR_EBP);
            break;
        case NDR_SI:
            set_ir1_opnd_base_reg_bd(ir1_opnd, NDR_ESI);
            break;
        case NDR_DI:
            set_ir1_opnd_base_reg_bd(ir1_opnd, NDR_EDI);
            break;
        case NDR_R8W:
            set_ir1_opnd_base_reg_bd(ir1_opnd, NDR_R8D);
            break;
        case NDR_R9W:
            set_ir1_opnd_base_reg_bd(ir1_opnd, NDR_R9D);
            break;
        case NDR_R10W:
            set_ir1_opnd_base_reg_bd(ir1_opnd, NDR_R10D);
            break;
        case NDR_R11W:
            set_ir1_opnd_base_reg_bd(ir1_opnd, NDR_R11D);
            break;
        case NDR_R12W:
            set_ir1_opnd_base_reg_bd(ir1_opnd, NDR_R12D);
            break;
        case NDR_R13W:
            set_ir1_opnd_base_reg_bd(ir1_opnd, NDR_R13D);
            break;
        case NDR_R14W:
            set_ir1_opnd_base_reg_bd(ir1_opnd, NDR_R14D);
            break;
        case NDR_R15W:
            set_ir1_opnd_base_reg_bd(ir1_opnd, NDR_R15D);
            break;
        default:
            lsassert(0);
            break;
        ...
        bswap_opnd =
            load_ireg_from_ir1_bd(ir1_opnd, UNKNOWN_EXTENSION, false);
        la_bstrins_w(bswap_opnd, zero_ir2_opnd, 15, 0);
    } else if (opnd_size == 32) {
        bswap_opnd = load_ireg_from_ir1_bd(ir1_opnd, UNKNOWN_EXTENSION, false);
    ...
    */
    ...
    store_ireg_to_ir1_bd(bswap_opnd, ir1_get_opnd_bd(pir1, 0), false);
 ...
 * @return false
 */
bool translate_shrx_bd(IR1_INST *pir1)
{
    IR1_OPND_BD *opnd0 = ir1_get_opnd_bd(pir1, 0);
    IR1_OPND_BD *opnd1 = ir1_get_opnd_bd(pir1, 1);
    ...
    IR2_OPND src2 = load_ireg_from_ir1_bd(opnd2, UNKNOWN_EXTENSION, false);
 ...
 * @return false
 */
bool translate_shlx_bd(IR1_INST *pir1)
{
    IR1_OPND_BD *opnd0 = ir1_get_opnd_bd(pir1, 0);
    IR1_OPND_BD *opnd1 = ir1_get_opnd_bd(pir1, 1);
    IR1_OPND_BD *opnd2 = ir1_get_opnd_bd(pir1, 2);
 ...
 * @return false
 */
bool translate_sarx_bd(IR1_INST *pir1)
{
    IR1_OPND_BD *opnd0 = ir1_get_opnd_bd(pir1, 0);
    IR1_OPND_BD *opnd1 = ir1_get_opnd_bd(pir1, 1);
    IR1_OPND_BD *opnd2 = ir1_get_opnd_bd(pir1, 2);
