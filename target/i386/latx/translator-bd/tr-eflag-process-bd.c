                        ...
                        IR2_OPND src1, IR1_INST *pir1)
{
    ...
    case ND_INS_SHLD: {
        int mask = ((ir1_opnd_size_bd(ir1_get_opnd_bd(pir1, 0)) == 64) ? 0x3f : 0x1f);
    ...
    case ND_INS_SHRD: {
        if (ir2_opnd_is_imm(&src1)) {
            lsassertm((ir2_opnd_imm(&src1) &
                ((ir1_opnd_size_bd(ir1_get_opnd_bd(pir1, 0)) == 64) ? 0x3f : 0x1f))
    ...
    case ND_INS_BLSI: {
        ...
        int opnd_size = ir1_opnd_size_bd(ir1_get_opnd_bd(pir1, 0));
...
static void generate_af(IR2_OPND dest, IR2_OPND src0,
                        IR2_OPND src1, IR1_INST *pir1)
{
    if (ir1_opcode_bd(pir1) == ND_INS_SHLD ||
        ir1_opcode_bd(pir1) == ND_INS_SHRD) {
...
static void generate_sf(IR2_OPND dest, IR2_OPND src0, IR2_OPND src1)
{
    ...
    int operation_size = ir1_opnd_size_bd(ir1_get_opnd_bd(pir1, 0));
                        ...
                        IR2_OPND src1, IR1_INST *pir1)
{
    ...
    switch (ir1_opcode_bd(pir1)) {
        case ND_INS_SHLD: {
            ...
            load_ireg_from_ir1_2_bd(dest_old, opnd0, ZERO_EXTENSION, false);
            ...
            if (opnd_size == 16) {
                ...
                load_ireg_from_ir1_2_bd(dest_old, opnd1, ZERO_EXTENSION, false);
            ...
            */
            ...
            la_srli_d(t_of_opnd, t_of_opnd, ir1_opnd_size_bd(ir1_get_opnd_bd(pir1, 0)) - OF_BIT_INDEX - 1);
        ...
        case ND_INS_SHRD: {
            ...
            if (opnd_size == 16) {
                ...
                load_ireg_from_ir1_2_bd(dest_old, opnd0, ZERO_EXTENSION, false);
                load_ireg_from_ir1_2_bd(src0_old, opnd1, ZERO_EXTENSION, false);
...
static void generate_cf_not_sx(IR2_OPND dest, IR2_OPND src0, IR2_OPND src1)
{
    ...
    case dt_X86_INS_CMP:
    case dt_X86_INS_SUB: {
        lsassert(ir1_opnd_size(ir1_get_opnd_bd(pir1, 0)) == 8 ||
                 ir1_opnd_size(ir1_get_opnd_bd(pir1, 0)) == 16 ||
                 ir1_opnd_size(ir1_get_opnd_bd(pir1, 0)) == 32);
        IR2_OPND cf_opnd = ra_alloc_itemp();
        if (ir1_opnd_size(ir1_get_opnd_bd(pir1, 0)) != 32)
            append_ir2_opnd2i(mips_dsra, cf_opnd, dest,
                              ir1_opnd_size(ir1_get_opnd_bd(pir1, 0)));
        else
    ...
    /* lsenv->tr_data->curr_tb->dump(); */
    lsassertm(0, "%s for %s is not implemented\n", __FUNCTION__,
              ir1_name_bd(lsenv->tr_data->curr_ir1_inst));
}
...
static void generate_of_not_sx(IR2_OPND dest, IR2_OPND src0, IR2_OPND src1)
{
    ...
    case dt_X86_INS_ADD:
    case dt_X86_INS_SUB: {
        lsassert(ir1_opnd_size(ir1_get_opnd_bd(pir1, 0)) == 32);
...
void generate_eflag_calculation_bd(IR2_OPND dest, IR2_OPND src0, IR2_OPND src1,
                                IR1_INST *pir1, bool flags)
{
    bool need_calc_flag = ir1_need_calculate_any_flag_bd(pir1);
    bool use_lbt = true;
    if (need_calc_flag) {
        use_lbt = generate_eflag_by_lbt_bd(dest, src0, src1, pir1, flags);
    }
    ...
    if (use_lbt) {
        if (ir1_opcode_bd(pir1) == ND_INS_SBB) {
            generate_af(dest, src0, src1, pir1);
        } else if (ir1_opcode_bd(pir1) == ND_INS_BLSI ||
                   ir1_opcode_bd(pir1) == ND_INS_BLSR ||
                   ir1_opcode_bd(pir1) == ND_INS_BLSMSK) {
    ...
    if (ir1_need_calculate_pf_bd(pir1))
        generate_pf(dest, src0, src1);
    if (ir1_need_calculate_af_bd(pir1))
        generate_af(dest, src0, src1, pir1);
    if (ir1_need_calculate_zf_bd(pir1))
        generate_zf(dest, src0, src1, pir1);
    if (ir1_need_calculate_sf_bd(pir1))
        generate_sf(dest, src0, src1);

    /* calculate cf and of separately */
    if (flags) {
        if (ir1_need_calculate_cf_bd(pir1))
            generate_cf(dest, src0, src1, pir1);
        if (ir1_need_calculate_of_bd(pir1))
